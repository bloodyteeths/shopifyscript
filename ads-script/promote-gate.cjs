...
class PromoteGate {
  ...
  async checkIdempotencyResults() {
    ...
    const logFiles = fs.readdirSync(logDir)
      .filter(file => file.includes('idempotency_test') && file.endsWith('.log'))
      .map(file => ({
        name: file,
    ...
    if (logFiles.length === 0) {
      if (this.options.requireIdempotencyTest) {
        throw new Error('No idempotency test results found - test required before promotion');
      } else {
    ...
    if (ageMs > this.options.maxLogAge) {
      throw new Error(`Latest idempotency test is too old (${Math.round(ageMs / 1000 / 60)} minutes ago)`);
    }
  ...
  async parseIdempotencyLog(logPath) {
    ...
    const testPassedMatch = logContent.match(/TEST_COMPLETE: Idempotency test (PASSED|FAILED)/);
    const firstRunMatch = logContent.match(/FIRST_RUN_COMPLETE: First run planned (\d+) mutations/);
    const secondRunMatch = logContent.match(/SECOND_RUN_COMPLETE: Second run planned (\d+) mutations/);
  ...
  makeGateDecision() {
    ...
    const testAge = this.results.testTimestamp ? 
      Date.now() - this.results.testTimestamp.getTime() : null;
    
    decision.checks.testRecency = {
      passed: testAge !== null && testAge <= this.options.maxLogAge,
      details: testAge ? `Test age: ${Math.round(testAge / 1000 / 60)} minutes` : 'No timestamp found'
    };

    // Check 3: No errors in test execution
    decision.checks.noErrors = {
      passed: this.results.errors.length === 0,
      details: this.results.errors.length > 0 ? 
        `${this.results.errors.length} errors found` : 'No errors detected'
    };

    // Check 4: Backend gate status
    decision.checks.backendGate = {
      passed: !this.options.requireBackendValidation || 
              (this.results.backendGateStatus && this.results.backendGateStatus.gateStatus === 'OPEN'),
      details: this.results.backendGateStatus ? 
        `Backend status: ${this.results.backendGateStatus.gateStatus}` : 'Backend validation disabled'
    };

    // Check 5: Label guard active
    decision.checks.labelGuard = {
      passed: !this.options.requireLabelGuard || this.results.labelGuardActive,
      details: this.results.labelGuardActive ? 'Label guard active' : 'Label guard status unclear'
    };
    ...
    decision.checks.warningsThreshold = {
      ...
      details: this.results.warnings.length > 0 ? 
        `${this.results.warnings.length} warnings detected` : 'No warnings'
    };
    ...
    if (decision.canPromote) {
      decision.summary = '✅ All safety checks passed - Script is safe for production deployment';
      ...
      decision.summary = '❌ Safety checks failed - DO NOT deploy to production';
      decision.recommendations = [
      ...
      if (!decision.checks.idempotencyTest.passed) {
        decision.recommendations.push('Check script for missing label guards or state detection');
      }
      
      if (!decision.checks.testRecency.passed) {
        decision.recommendations.push('Run fresh idempotency tests before deployment');
      }
      
      if (!decision.checks.noErrors.passed) {
        decision.recommendations.push('Investigate and resolve test execution errors');
      }
    }
  ...
  writeGateDecisionLog(decision) {
    ...
    const logFile = path.join(this.options.logDirectory, `${timestamp}_promote_gate.log`);
  ...
  async checkLabelGuardStatus() {
    try {
      ...
      if (!fs.existsSync(logDir)) {
        this.results.warnings.push('Log directory not found for label guard check');
        return;
      }
      ...
      for (const file of logFiles) {
        ...
        if (content.includes('LABEL_GUARD') || content.includes('PROOFKIT_AUTOMATED')) {
      ...
      if (labelGuardFound) {
        ...
        this.results.warnings.push('Label guard status unclear from recent logs');
  ...
  async checkReservedKeywords() {
    try {
      ...
      if (recentLogs.length === 0) {
        this.results.warnings.push('No recent logs found for keyword protection check');
        return;
      }
      ...
      for (const keyword of reservedKeywords) {
        if (content.includes(`NEG_GUARD: Blocked reserved keyword`) || 
      ...
      if (protectionActive) {
        ...
        this.results.warnings.push('Reserved keyword protection not detected in recent logs');
        console.warn('⚠️  Reserved keyword protection status unclear');
      }
      
    } catch (error) {
      this.results.errors.push(`Reserved keyword check failed: ${error.message}`);
  ...
  async checkMutationLimits() {
    try {
      ...
      if (currentMutations > maxMutations) {
        this.results.errors.push(`Mutation count ${currentMutations} exceeds limit ${maxMutations}`);
        console.error(`❌ Mutation limit exceeded: ${currentMutations}/${maxMutations}`);
      } else {
      ...
      if (currentMutations > 50) {
        this.results.warnings.push(`High mutation count detected: ${currentMutations}`);
