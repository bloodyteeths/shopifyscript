/** Ads Autopilot AI - Google Ads Script (Production Ready)
...
function getConfig_() {
  ...
  try {
    ...
    if (code >= 200 && code < 300) {
      ...
      if (parsed && parsed.secret_rotation) {
        log_('! Backend signaling secret rotation required');
        handleSecretRotation_(parsed.secret_rotation);
      }
    ...
    if (code === 401 || code === 403) {
      ...
      if (rotated) {
        ...
        if (r2.getResponseCode() >= 200 && r2.getResponseCode() < 300) {
          ...
          var parsed2 = null;
          try { parsed2 = JSON.parse(r2.getContentText()); } catch(e) { return null; }
          return parsed2 && parsed2.config ? parsed2.config : null;
        }
      }
    }
