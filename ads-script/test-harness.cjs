...
class AdsScriptTestHarness {
  ...
  async runIdempotencyTest(scriptMainFunction) {
    ...
    try {
      ...
      const firstRunResults = await this.runScriptPreview(
        scriptMainFunction,
        "first",
      );
      ...
      this.log(
        "FIRST_RUN_COMPLETE",
        `First run planned ${firstRunResults.mutationCount} mutations`,
      );

      // Second run in preview mode
      this.log("SECOND_RUN_START", "Starting second preview run");
      const secondRunResults = await this.runScriptPreview(
        scriptMainFunction,
        "second",
      );
      ...
      this.log(
        "SECOND_RUN_COMPLETE",
        `Second run planned ${secondRunResults.mutationCount} mutations`,
      );

      // Validate idempotency
      const isIdempotent = this.validateIdempotency(
        firstRunResults,
        secondRunResults,
      );
      ...
      this.log(
        "TEST_COMPLETE",
        `Idempotency test ${isIdempotent ? "PASSED" : "FAILED"}`,
      );

      if (!isIdempotent) {
        this.log(
          "TEST_FAILURE_DETAIL",
          this.generateFailureReport(firstRunResults, secondRunResults),
        );
      }
  ...
  async runScriptPreview(scriptFunction, runLabel) {
    ...
    try {
      ...
      this.log(
        `${runLabel.toUpperCase()}_MUTATIONS`,
        JSON.stringify(results.mutations),
      );
  ...
  setupPreviewMocks() {
    ...
    if (typeof AdsApp !== "undefined" && AdsApp.campaigns) {
      ...
      originalMocks.audienceBuild = this.mockMethod(
  ...
  validateIdempotency(firstRun, secondRun) {
    ...
    if (secondMutationTypes.size > 0) {
      const unexpectedTypes = [...secondMutationTypes].filter(
        (type) => !firstMutationTypes.has(type),
      );
