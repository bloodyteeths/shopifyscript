...
function mockMainFunction() {
  ...
  if (mockMainFunction.callCount === 0) {
    ...
    mockMainFunction.plannedMutations = [
      { type: 'BUDGET_CHANGE', details: { campaign: 'Test Campaign', amount: 50 } },
      { type: 'RSA_CREATE', details: { campaign: 'Test Campaign', adGroup: 'Test AdGroup' } }
    ];
  } else {
...
async function runIdempotencyTest() {
  ...
  try {
    ...
    console.log(`First Run Mutations: ${testResults.firstRun?.mutationCount || 'N/A'}`);
    console.log(`Second Run Mutations: ${testResults.secondRun?.mutationCount || 'N/A'}`);
    ...
    if (!testResults.passed) {
      ...
      console.log(JSON.stringify(testResults.secondRun?.mutations || [], null, 2));
    }
