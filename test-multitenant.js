...
function sign(payload) {
  return crypto.createHmac('sha256', HMAC_SECRET).update(payload).digest('base64').replace(/=+$/, '');
}
...
async function testTenant(tenantName) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/config?tenant=${tenantName}&sig=${sig}`);
