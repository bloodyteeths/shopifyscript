...
describe('ProofKit Demo Funnel Screenshots', () => {
  ...
  it('should capture complete funnel flow screenshots', () => {
    ...
    stepCounter++;
    cy.screenshot(`e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, '0')}-promote-confirmation-modal`, { capture: 'fullPage' });

    cy.get('[data-testid="confirm-promote"]').click();
    cy.get('[data-testid="promote-confirmation-modal"]').invoke('hide');
    
    stepCounter++;
    cy.screenshot(`e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, '0')}-setup-completion-success`, { capture: 'fullPage' });

    // Generate the step documentation data
    cy.task('appendFunnelStep', {
      stepNumber: 1,
      label: 'App Installation Landing',
      expect: 'User sees ProofKit installation page with clear value proposition and install button',
      route: '/install',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Settings form loads with required fields and validation indicators',
      route: '/app/settings',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Wizard configures budget caps, CPC ceilings, and safety exclusions',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Script preview shows planned mutations, second run confirms idempotency',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'AI generates RSA content with 30/90 character validation and quality checks',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'High-intent modal displays urgent messaging with countdown timer',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Audience setup with upload instructions and size guard protection',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'PROMOTE warning, schedule configuration, and activation controls',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Confirmation modal warns about live changes requiring explicit approval',
      route: '/app/funnel/wizard',
    ...
    cy.task('appendFunnelStep', {
      ...
      expect: 'Complete funnel success with automation active and dashboard access',
      route: '/app/funnel/wizard',
