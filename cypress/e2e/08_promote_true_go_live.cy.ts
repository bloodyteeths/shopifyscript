...
describe('PROMOTE Enable & Go Live', () => {
  ...
  it('should enable PROMOTE and execute live automation', function() {
    ...
    cy.get('[data-testid="promote-warning"]').should('contain.text', 'live changes');
    cy.get('[data-testid="promote-toggle"]').should('be.visible').and('not.be.disabled');
    
    // Step 1: Configure schedule window (optional)
    const scheduleTime = new Date(Date.now() + 300000).toISOString().slice(0, 16); // +5 minutes
    ...
    cy.get('[data-testid="schedule-summary"]').should('contain.text', '60 minutes');
    ...
    cy.get('[data-testid="live-changes-warning"]').should('contain.text', 'real budget');
    ...
    cy.get('[data-testid="promote-enabled-badge"]').should('contain.text', 'ACTIVE');
    ...
    cy.intercept('GET', '/api/execution/status*', {
      ...
      body: {
        ...
        mutations: [
          { type: 'BUDGET_CHANGE', status: 'COMPLETED', campaign: 'Test Campaign' },
          { type: 'CPC_CEILING', status: 'COMPLETED', campaign: 'Test Campaign' },
          { type: 'SCHEDULE_ADD', status: 'COMPLETED', campaign: 'Test Campaign' }
        ]
      }
    }).as('executionStatus');
    ...
    cy.get('[data-testid="mutations-applied"]').should('contain.text', '3 changes applied');
  ...
  it('should handle PROMOTE failures and safety fallbacks', () => {
    ...
    cy.get('[data-testid="safety-requirements"]').should('contain.text', 'Idempotency test');
  ...
  it('should demonstrate emergency PROMOTE disable', () => {
    ...
    cy.get('[data-testid="emergency-warning"]').should('contain.text', 'immediately stop');
    ...
    cy.get('[data-testid="automation-stopped"]').should('contain.text', 'stopped');
  });
  ...
  it('should validate schedule window constraints', () => {
    ...
    cy.get('[data-testid="schedule-error"]').should('contain.text', 'future time');
      ...
      req.reply((res) => {
        ...
        res.send({
          ...
          body: {
            ok: true,
            progress: progress,
            currentStep: progress < 50 ? 'Applying budget caps' : 'Validating RSA assets',
