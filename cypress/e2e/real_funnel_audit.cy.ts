...
describe('ProofKit Real Funnel Audit & Screenshots', () => {
  ...
  it('should audit the complete ProofKit funnel with real UI interactions', () => {
    ...
    const sections = [
      { path: '/app/autopilot', name: 'Autopilot', description: 'Automated campaign management interface' },
      { path: '/app/insights', name: 'Insights', description: 'Performance insights and analytics dashboard' },
      { path: '/app/intent-os', name: 'Intent OS', description: 'Intent-based content optimization interface' },
      { path: '/app/advanced', name: 'Advanced', description: 'Advanced configuration and settings' }
    ];
    ...
    cy.get('body').then($body => {
      ...
      const modalTriggers = $body.find('[data-testid*="modal"], [data-testid*="open"], button[title*="open"]');
      ...
      if (modalTriggers.length > 0) {
        ...
        cy.get('body').then($bodyWithModal => {
          if ($bodyWithModal.find('[data-testid*="close"], button[title*="close"]').length > 0) {
            cy.get('[data-testid*="close"], button[title*="close"]').first().click({ force: true });
          } else if ($bodyWithModal.find('.Polaris-Modal-CloseButton').length > 0) {
    ...
    cy.get('body').then($body => {
      ...
      if ($body.find('input[type="text"]:visible').length > 0) {
        cy.get('input[type="text"]:visible').first().type('test-value', { force: true });
        cy.wait(500);
