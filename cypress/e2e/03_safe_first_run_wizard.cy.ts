...
describe('Safe First Run Wizard', () => {
  ...
  it('should complete safe first run configuration', function() {
    ...
    cy.get('[data-testid="wizard-budget-cap"]').clear().type(safeRunDefaults.dailyBudgetCap);
    ...
    cy.get('[data-testid="wizard-budget-cap"]').should('have.value', safeRunDefaults.dailyBudgetCap);
    
    // Step 3: Configure CPC ceiling
    cy.get('[data-testid="wizard-cpc-ceiling"]').clear().type(safeRunDefaults.cpcCeiling);
    ...
    cy.get('[data-testid="wizard-schedule-start"]').clear().type(safeRunDefaults.scheduleStart);
    cy.get('[data-testid="wizard-schedule-end"]').clear().type(safeRunDefaults.scheduleEnd);
    ...
    cy.get('[data-testid="wizard-exclusions"]').type(safeRunDefaults.exclusions);
    ...
    cy.get('[data-testid="wizard-progress"]').should('contain.text', 'Script Preview');
  ...
  it('should validate safe run limits and show warnings', function() {
    ...
    cy.get('[data-testid="budget-warning"]').should('contain.text', 'Consider starting with a lower budget');
    
    // Test extreme CPC warning
    cy.get('[data-testid="wizard-cpc-ceiling"]').type('10.00');
    
    cy.get('[data-testid="cpc-warning"]').should('be.visible');
    cy.get('[data-testid="cpc-warning"]').should('contain.text', 'High CPC ceiling');
  });
