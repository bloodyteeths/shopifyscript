...
describe('Script Preview & Idempotency Testing', () => {
  ...
  it('should run script preview and verify idempotency', function() {
    ...
    cy.get('[data-testid="preview-run"]').should('be.visible').and('not.be.disabled');
    cy.get('[data-testid="preview-explanation"]').should('contain.text', 'no live changes');
    ...
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'Budget capped');
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'CPC ceiling');
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'Schedule added');
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'negatives attached');
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'RSA assets validated');
    cy.get('[data-testid="mutation-list"]').should('contain.text', 'Audience attached');
    ...
    cy.get('[data-testid="idempotency-badge"]').should('contain.text', 'passed');
    cy.get('[data-testid="no-mutations-message"]').should('contain.text', 'no duplicate mutations');
  ...
  it('should handle preview failures gracefully', () => {
    ...
    cy.get('[data-testid="error-message"]').should('contain.text', 'configuration invalid');
  ...
  it('should validate mutation expectations', function() {
    ...
    expectedMutations.forEach(mutationType => {
      cy.get('[data-testid="mutation-list"]')
        .should('contain.text', mutationType.replace('_', ' ').toLowerCase());
    });
