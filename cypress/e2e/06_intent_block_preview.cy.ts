...
describe('Intent Block Preview & UTM Testing', () => {
  ...
  it('should preview intent blocks for different UTM terms', function() {
    ...
    cy.get('[data-testid="intent-preview-explanation"]').should('contain.text', 'content changes');
    ...
    cy.get('[data-testid="preview-headline"]').should('contain.text', 'Limited Time');
    ...
    cy.get('[data-testid="preview-headline"]').should('contain.text', 'Discover');
    cy.get('[data-testid="educational-benefits"]').should('be.visible');
    cy.get('[data-testid="expert-social-proof"]').should('contain.text', 'experts');
    ...
    cy.get('[data-testid="preview-headline"]').should('contain.text', 'Why We\'re #1');
    cy.get('[data-testid="competitive-badge"]').should('contain.text', 'BEST VALUE');
    ...
    cy.get('[data-testid="preview-headline"]').should('contain.text', 'Welcome Back');
    cy.get('[data-testid="incentive-badge"]').should('contain.text', 'SAVE 15%');
    cy.get('[data-testid="personal-message"]').should('contain.text', 'waiting');
    
    cy.get('[data-testid="close-preview"]').click();
    
    // Step 5: Verify UTM parameter understanding
    cy.get('[data-testid="utm-explanation"]').should('contain.text', 'automatically adapts');
  ...
  it('should handle intent preview failures', () => {
    ...
    cy.get('[data-testid="intent-preview-modal"]').should('be.visible');
    cy.get('[data-testid="preview-error"]').should('contain.text', 'service unavailable');
  ...
  it('should validate UTM parameter format and tracking', () => {
    ...
    cy.get('[data-testid="utm-preview-result"]').should('contain.text', 'high-intent');
    ...
    cy.get('[data-testid="utm-format-help"]').should('contain.text', 'Use lowercase letters');
  });
