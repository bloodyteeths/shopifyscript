...
describe('Audience Attachment & Size Guards', () => {
  ...
  it('should configure audience attachment with size guards', function() {
    ...
    cy.get('[data-testid="upload-step-1"]').should('contain.text', 'Export your audience CSV');
    cy.get('[data-testid="upload-step-6"]').should('contain.text', 'Paste the ID');
    ...
    cy.get('[data-testid="audience-mode"]').should('have.value', 'OBSERVE');
    cy.get('[data-testid="mode-explanation"]').should('contain.text', 'Recommended for new lists');
    ...
    cy.get('[data-testid="size-guard-status"]').should('contain.text', 'Above minimum');
    
    // Step 5: Size guard demonstration
    cy.get('[data-testid="size-guard-explanation"]').should('be.visible');
    cy.get('[data-testid="size-guard-explanation"]').should('contain.text', '1,000 user minimum');
  ...
  it('should handle small audience lists with size guard warnings', function() {
    ...
    cy.get('[data-testid="bid-modifier-skipped"]').should('contain.text', 'skipped');
  ...
  it('should validate audience mode configurations', () => {
    ...
    cy.get('[data-testid="target-mode-warning"]').should('contain.text', 'reduces reach');
    ...
    cy.get('[data-testid="bid-modifier-warning"]').should('contain.text', 'High bid modifier');
    ...
    cy.get('[data-testid="exclude-explanation"]').should('contain.text', 'never shown');
  });
  ...
  it('should demonstrate keyboard accessibility for audience forms', () => {
    ...
    // Verify ARIA labels
    cy.get('[data-testid="audience-list-id"]').should('have.attr', 'aria-label');
