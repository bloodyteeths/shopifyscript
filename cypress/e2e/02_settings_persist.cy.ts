...
describe("ProofKit Settings Configuration", () => {
  ...
  it("should persist settings and enable wizard access", function () {
    ...
    cy.get('[data-testid="settings-google-ads-id"]').type(
      demoTenant.googleAdsId,
    );
    cy.get('[data-testid="settings-conversion-label"]').type(
      demoTenant.conversionLabel,
    );
    ...
    cy.get('[data-testid="continue-wizard-button"]')
      .should("be.visible")
      .and("not.be.disabled");
  ...
  it("should handle API errors with retry functionality", () => {
    ...
    cy.fixture("tenant-data").then((testData) => {
      ...
      cy.get('[data-testid="settings-backend-url"]').type(
        demoTenant.backendUrl,
      );
      cy.get('[data-testid="settings-hmac-secret"]').type(
        demoTenant.hmacSecret,
      );
