...
describe('AI Drafts Generation & Validation', () => {
  ...
  it('should generate and validate RSA drafts with 30/90 character limits', function() {
    ...
    cy.get('[data-testid="rsa-explanation"]').should('contain.text', '30/90 character');
    ...
    cy.get('[data-testid="generate-ai-drafts"]').should('contain.text', 'Generating...');
    ...
    cy.get('[data-testid="description-1-length"]').should('contain.text', '/90');
    cy.get('[data-testid="description-1-badge"]').should('have.class', 'success'); // Valid length
    
    // Step 3: Content quality validation
    cy.get('[data-testid="content-quality-check"]').should('be.visible');
    cy.get('[data-testid="deduplication-status"]').should('contain.text', 'No duplicates found');
  ...
  it('should handle character limit violations', () => {
    ...
    cy.get('[data-testid="description-1-badge"]').should('have.class', 'critical');
  ...
  it('should validate deduplication across multiple drafts', () => {
    ...
    cy.get('[data-testid="duplicate-count"]').should('contain.text', '2 duplicates');
    cy.get('[data-testid="approve-drafts"]').should('be.disabled');
