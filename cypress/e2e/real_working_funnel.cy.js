...
describe("ProofKit Real Working Funnel Screenshots", () => {
  ...
  it("should capture working ProofKit interface across all sections", () => {
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-main-dashboard-working`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-autopilot-interface`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    cy.get("body").then(($body) => {
      ...
      if ($body.find('input[name="budget"]').length > 0) {
        ...
        stepCounter++;
        cy.screenshot(
          `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-autopilot-budget-input`,
          {
            capture: "fullPage",
            overwrite: true,
          },
      ...
      if ($body.find('select[name="objective"]').length > 0) {
        ...
        stepCounter++;
        cy.screenshot(
          `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-autopilot-objective-selection`,
          {
            capture: "fullPage",
            overwrite: true,
          },
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-insights-terms-analysis`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    cy.get("body").then(($body) => {
      if ($body.find('input[name="q"]').length > 0) {
        ...
        stepCounter++;
        cy.screenshot(
          `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-insights-terms-filter`,
          {
            capture: "fullPage",
            overwrite: true,
          },
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-tablet-responsive-view`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-mobile-responsive-view`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    cy.get("body").then(($body) => {
      ...
      if ($body.find("button").length > 0) {
        ...
        if (previewButton.length > 0) {
          ...
          stepCounter++;
          cy.screenshot(
            `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-button-interaction-preview`,
            {
              capture: "fullPage",
              overwrite: true,
            },
    ...
    stepCounter++;
    cy.screenshot(
      `e2e-test-results/screenshots/funnel/${stepCounter.toString().padStart(2, "0")}-final-working-overview`,
      {
        capture: "fullPage",
        overwrite: true,
      },
    ...
    cy.writeFile("e2e-test-results/docs/working-funnel-steps.json", [
      ...
      },
      {
        ...
        route: "/app/autopilot",
        description:
          "Campaign automation interface with forms and controls working",
