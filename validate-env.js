...
function validateBackend() {
  ...
  for (const varName of requiredVars) {
    ...
    if (value) {
      log(`✅ ${varName}: ${varName.includes('PRIVATE_KEY') ? '[REDACTED]' : value.length > 50 ? value.slice(0, 50) + '...' : value}`, 'green');
    } else {
  ...
  try {
    ...
    log(`✅ TENANT_REGISTRY_JSON: Valid JSON with ${tenantCount} tenants`, 'green');
  } catch (error) {
...
function main() {
  ...
  log(`Backend Environment: ${backendValid ? '✅ VALID' : '❌ ISSUES FOUND'}`, backendValid ? 'green' : 'red');
  log(`Shopify UI Environment: ${uiValid ? '✅ VALID' : '❌ ISSUES FOUND'}`, uiValid ? 'green' : 'red');
  log(`Cross-App Communication: ${communicationValid ? '✅ VALID' : '❌ ISSUES FOUND'}`, communicationValid ? 'green' : 'red');
  
  const overallValid = backendValid && uiValid && communicationValid;
  
  log(`\nOverall Status: ${overallValid ? '✅ ALL SYSTEMS GO' : '❌ CONFIGURATION ISSUES DETECTED'}`, overallValid ? 'green' : 'red');
