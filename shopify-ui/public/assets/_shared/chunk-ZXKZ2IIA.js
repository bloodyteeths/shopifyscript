...
function sanitizeCustomProperties(styles58) {
  const nonNullValues = Object.entries(styles58).filter(([_, value]) => value != null);
...
function Cell2({
  ...
  const className = classNames(styles57.Cell, columnSpan?.xs && styles57[`Cell-${columnSpan.xs}-column-xs`], columnSpan?.sm && styles57[`Cell-${columnSpan.sm}-column-sm`], columnSpan?.md && styles57[`Cell-${columnSpan.md}-column-md`], columnSpan?.lg && styles57[`Cell-${columnSpan.lg}-column-lg`], columnSpan?.xl && styles57[`Cell-${columnSpan.xl}-column-xl`]);
  const style = {
    gridArea,
