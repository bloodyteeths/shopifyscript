...
export default function SimpleLines({ data }: { data: any[] }) {
  ...
  React.useEffect(() => {
    let alive = true;
    import('recharts').then(mod => { if (alive) setR(mod); });
    return () => { alive = false; };
  }, []);

  if (!R) {
    return <div style={{height:240, border:'1px solid #eee', borderRadius:8, padding:8}} />;
  }

  const { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } = R;
  return (
    <div style={{height:240, border:'1px solid #eee', borderRadius:8, padding:8}}>
