...
export function ShopSetupTest() {
  const [testMode, setTestMode] = React.useState<'banner' | 'config' | 'input'>('banner');
  ...
  const testCases = [
    { name: 'Valid Names', values: ['proofkit', 'my-store', 'test123', 'awesome_shop'] },
    { name: 'Invalid Names', values: ['', 'a', 'shop name', 'shop@name', '-invalid', 'toolongshopnamethatexceeds64charactersandshouldbeinvalidaccordingtorules'] }
  ];
    ...
    <div style={{ padding: '20px', maxWidth: '800px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'monospace', fontSize: '14px' }}>
          <div>Stored Shop Name: <strong>{status.stored || 'null'}</strong></div>
          <div>Current Shop Name: <strong>{status.current}</strong></div>
          <div>Setup Needed: <strong style={{ color: status.needsSetup ? '#dc3545' : '#28a745' }}>
