...
export default function TermsExplorer() {
  ...
  const applyNegatives = React.useCallback(
    async (terms: string[]) => {
      if (isProcessing || !Array.isArray(terms) || terms.length === 0) return;
      ...
      try {
        ...
        const actions = terms
          ...
          .map((t) => ({ type: "add_exact_negative", target: t }));
  ...
  const removeNegative = React.useCallback(
    async (term: string) => {
      if (isProcessing || !term || typeof term !== "string") return;
      ...
      try {
        ...
        const r = await backendFetch("/insights/actions/apply", "POST", {
          nonce: Date.now(),
          actions: [{ type: "remove_exact_negative", target: term.trim() }],
        });
        const ok = r?.json?.ok;
        const applied = r?.json?.applied?.length || 0;
        const skipped = r?.json?.skipped?.length || 0;
        setToast(
          ok ? `Applied ${applied}, skipped ${skipped}` : "Failed to remove",
        );
      } catch (error) {
    ...
    <div>
      ...
      >
        ...
        <input
          ...
          step="1"
          min="0"
          defaultValue={sp.get("min_clicks") || "0"}
        />
        <input
          name="min_cost"
          type="number"
          step="0.01"
          min="0"
          defaultValue={sp.get("min_cost") || "0"}
        />
        ...
        <button
          onClick={() => {
            const candidates = rows
              .filter(
                (r: any) => r && !r.is_negative && selected.current.has(r.term),
              )
              .map((r: any) => r.term);
            return applyNegatives(candidates);
          }}
          disabled={nav.state !== "idle" || isProcessing}
        >
          {isProcessing ? "Processing..." : "Add exact negative (selected)"}
        </button>
        <a href={toCsvHref}>
          <button type="button">Export CSV (current filters)</button>
        </a>
        {!!toast && (
          <span role="status" style={{ fontSize: 12, opacity: 0.8 }}>
            {toast}
          </span>
        )}
      </div>
      ...
      >
        <thead>
          <tr>
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.currentTarget.checked) {
                    rows.forEach((r: any) => selected.current.add(r.term));
                  } else {
            ...
            {["term", "clicks", "cost", "conversions", "cpc", "cpa"].map(
              (col) => {
                const is = (sp.get("sort") || "cost") === col;
                const nextDir =
                  is && (sp.get("dir") || "desc") === "desc" ? "asc" : "desc";
                const u = new URLSearchParams(Array.from(sp.entries()));
                u.set("sort", col);
                u.set("dir", nextDir);
                u.set("page", "1");
                return (
                  <th key={col} align={col === "term" ? "left" : "right"}>
                    <a
                      href={`?${u.toString()}`}
        ...
        <tbody>
          {rows.map((r: any) => {
            if (!r || typeof r !== "object" || !r.term) return null;
            return (
              <tr key={`row-${r.term}-${r.clicks || 0}`}>
                <td>
                  <input
                    type="checkbox"
                    disabled={r.is_negative}
                    onChange={(e) => {
                      if (e.currentTarget.checked) selected.current.add(r.term);
                      else selected.current.delete(r.term);
                    }}
                  />
                </td>
                <td>
                ...
                <td align="right">
                  {!r.is_negative ? (
                    <button
                      onClick={() => applyNegatives([r.term])}
                      disabled={isProcessing}
                    >
                      {isProcessing ? "Processing..." : "Add exact negative"}
                    </button>
                  ) : (
                    <button
                      onClick={() => removeNegative(r.term)}
                      disabled={isProcessing}
                    >
          ...
          {!rows.length && (
            <tr>
              <td colSpan={8}>No rows match your filters.</td>
            </tr>
          )}
        </tbody>
      </table>
      ...
      >
        {(() => {
          ...
          return (
            <>
              <span>
                {page} / {pages} ({total} rows)
              </span>
              <a href={`?${prev.toString()}`}>
