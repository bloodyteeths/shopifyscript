...
export default function AudiencesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const j = await r.json();
        if (j?.ok) setCfg(j.config || { plan: "starter" });
      } catch {}
    })();
  }, []);
  ...
  async function build() {
    ...
    const ok = r.ok;
    setToast(ok ? "Build requested" : "Build failed");
    if (ok) load();
  }
  ...
  return (
    <AppShell>
      <div style={{ padding: 16 }}>
        ...
        {toast && <p>{toast}</p>}
        {cfg?.plan !== "growth" && (
          <p style={{ color: "#888" }}>Upgrade to Growth to build segments.</p>
        )}
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input
            value={segmentsCsv}
            onChange={(e) => setSegmentsCsv(e.target.value)}
            placeholder="segments (csv)"
          />
          <button onClick={build} disabled={cfg?.plan !== "growth"}>
            Build
          </button>
        </div>
        <table
          ...
          <tbody>
            {exportsList.map((row, idx) => (
              <tr key={idx}>
                ...
                <td>{row.status}</td>
                <td>
                  {row.cm_ui_unhashed_csv ? (
                    <a
                      href={row.cm_ui_unhashed_csv}
                      target="_blank"
                      rel="noreferrer"
                    >
                      download
                ...
                <td>
                  {row.cm_api_hashed_csv ? (
                    <a
                      href={row.cm_api_hashed_csv}
                      target="_blank"
                      rel="noreferrer"
                    >
                      download
