...
export default function AIPage() {
  ...
  async function run() {
    const r = await fetch("/app/api/ai/writer", {
      ...
      body: JSON.stringify({ nonce: Date.now(), dryRun: true, limit: 20 }),
    });
  ...
  async function accept() {
    const items = (data.library || [])
      ...
      .map((r) => ({
        theme: r.theme,
        headlines_pipe: (r.headlines || []).join("|"),
        descriptions_pipe: (r.descriptions || []).join("|"),
        source: r.source || "ui",
      }));
    const r = await fetch("/app/api/ai/accept", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ nonce: Date.now(), items }),
    });
    const j = await r.json();
    setToast(j.ok ? `Accepted ${j.accepted}` : "Accept failed");
    if (j.ok) load();
  }
  ...
  return (
    <AppShell>
          ...
          <tbody>
            {data.rsa_default.map((r, idx) => (
              <tr key={idx}>
                ...
                <td>
                  {r?.lint?.ok ? "OK" : (r?.lint?.errors || []).join(",")}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={{ marginBottom: 8 }}>
        <button onClick={accept} disabled={!Object.values(sel).some(Boolean)}>
          Accept
        </button>
      </div>
              ...
              <tr key={idx}>
                <td>
                  <input
                    ...
                    checked={!!sel[idx]}
                    onChange={(e) =>
                      setSel({ ...sel, [idx]: e.target.checked })
                    }
                  />
                </td>
                <td>{r.theme}</td>
