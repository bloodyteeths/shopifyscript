...
export default function BillingPage() {
  ...
  async function loadCurrentSubscription() {
    try {
      setLoading(true);
      const response = await fetch('/api/billing/shopify/subscription/' + window.location.hostname, {
        method: 'GET',
        headers: {
  ...
  async function changePlan(newTierIndex) {
    ...
    try {
      ...
      const response = await fetch(`/api/billing/shopify/subscription/${currentSubscription.id}/change`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  async function cancelSubscription() {
    ...
    try {
      ...
      const response = await fetch(`/api/billing/shopify/subscription/${currentSubscription.id}/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  function getCurrentTier() {
    ...
    return PRICING_TIERS.find(tier => tier.price === parseFloat(currentSubscription.amount));
  }
                ...
                <div style={{ padding: '1rem' }}>
                  <Stack distribution="equalSpacing" alignment="center">
                    <Stack vertical spacing="tight">
                      <Text as="h3" variant="headingMd">{currentTier?.name || 'Unknown Plan'}</Text>
                      <Text as="p" variant="bodyMd" color="subdued">
                        ${currentSubscription.amount}/month
                      </Text>
                      <Badge status={currentSubscription.status === 'ACTIVE' ? 'success' : 'critical'}>
          ...
          <Layout.Section>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
              {PRICING_TIERS.map((tier, index) => {
                ...
                const isUpgrade = currentTier && PRICING_TIERS.findIndex(t => t.id === currentTier.id) < index;
                const isDowngrade = currentTier && PRICING_TIERS.findIndex(t => t.id === currentTier.id) > index;
                  ...
                  <Card key={tier.id} title={tier.name}>
                    <div style={{ padding: '1rem' }}>
                      <Stack vertical spacing="loose">
                        <Stack distribution="equalSpacing" alignment="center">
                          <Text as="h2" variant="headingLg">${tier.price}</Text>
                          {tier.popular && <Badge status="info">Popular</Badge>}
                          {isCurrentTier && <Badge status="success">Current Plan</Badge>}
                        </Stack>
                        ...
                        <div>
                          <Text as="h4" variant="headingXs">Features:</Text>
                        ...
                        <div>
                          <Text as="h4" variant="headingXs">Limits:</Text>
                          <DataTable
