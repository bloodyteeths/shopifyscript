...
export default function CanaryWizard() {
  ...
  async function attachAudience() {
    ...
    const r = await fetch("/app/api/audiences/mapUpsert", {
      method: "POST",
  ...
  async function aiDryRun() {
    ...
    const r = await fetch("/app/api/ai/writer", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body),
    });
  ...
  async function promoteWindow() {
    ...
    const r = await fetch("/app/api/promote/window", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body),
    });
    ...
    <AppShell>
      <div style={{ padding: 16, maxWidth: 800 }}>
        ...
        {status && status.ok && (
          ...
          >
            <strong>Status:</strong> {status?.window?.state || "inactive"} ·
            PROMOTE: {String(!!status?.promote)}
            {status?.window?.start && (
              <div>
                Window: {new Date(status.window.start).toLocaleString()} →{" "}
                {new Date(status.window.end || 0).toLocaleString()}
              </div>
            )}
          </div>
        )}
        ...
        <ol>
          ...
          <li>
            ...
            <textarea
              placeholder="Exclusions (other campaign names CSV)"
              value={exclusionsCsv}
              onChange={(e) => setExclusionsCsv(e.target.value)}
            />
        ...
        <p>
          Preview logs should include: Budget capped, Bidding, Schedule, Master
          negatives, N-gram miner, RSA build (draft gate), Audience attach. When
          PROMOTE off → no ad/keyword mutations.
        </p>
      </div>
    </AppShell>
  );
