              ...
              <FormLayout>
                <FormLayout.Group>
                  <TextField
                    ...
                    onChange={(value) => setSafeRunConfig(prev => ({ ...prev, dailyBudgetCap: value }))}
                    prefix="$"
                    type="number"
                    step="0.01"
                  ...
                  <TextField
                    ...
                    onChange={(value) => setSafeRunConfig(prev => ({ ...prev, cpcCeiling: value }))}
                    prefix="$"
                    type="number"
                    step="0.01"
                ...
                <FormLayout.Group>
                  <TextField
                    ...
                    onChange={(value) => setSafeRunConfig(prev => ({ ...prev, scheduleStart: value }))}
                    type="time"
                  ...
                  <TextField
                    ...
                    onChange={(value) => setSafeRunConfig(prev => ({ ...prev, scheduleEnd: value }))}
                    type="time"
                ...
                <TextField
                  ...
                  onChange={(value) => setSafeRunConfig(prev => ({ ...prev, exclusions: value }))}
  ...
  function ScriptPreviewStep() {
    ...
    return (
      <Card>
        <Card.Section>
          <Stack vertical>
            <TextStyle variation="strong">Script Preview (No Live Changes)</TextStyle>
            ...
            {previewResults && (
              <Card sectioned>
                <Stack vertical>
                  <TextStyle variation="strong">First Preview Results:</TextStyle>
                  <List>
                    {previewResults.mutations?.map((mutation: any, index: number) => (
                      <List.Item key={index}>
                        <Badge status={mutation.type === 'error' ? 'critical' : 'success'}>
                          {mutation.type}
                        </Badge>
            ...
            {secondPreviewResults && (
              <Card sectioned>
                <Stack vertical>
                  <TextStyle variation="strong">Second Preview Results (Should be empty):</TextStyle>
  ...
  function AiDraftsStep() {
    ...
    const generateDrafts = async () => {
      ...
      try {
        ...
        setError('Draft generation failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
      } finally {
    ...
    return (
      <Card>
        <Card.Section>
          <Stack vertical>
            <TextStyle variation="strong">AI-Generated RSA Drafts (Optional)</TextStyle>
                  ...
                  {aiDrafts.map((draft, index) => (
                    <Card key={index} sectioned>
                      <Stack vertical spacing="tight">
                        <TextStyle variation="strong">Headlines ({draft.headlines?.length || 0}):</TextStyle>
                        {draft.headlines?.map((headline: string, i: number) => (
                          <p key={i}>
                            <Badge status={headline.length <= 30 ? 'success' : 'critical'}>
                              {headline.length}/30
                            </Badge>
                            {' '}{headline}
                          </p>
                        ...
                        <TextStyle variation="strong">Descriptions ({draft.descriptions?.length || 0}):</TextStyle>
                        {draft.descriptions?.map((desc: string, i: number) => (
                          <p key={i}>
                            <Badge status={desc.length <= 90 ? 'success' : 'critical'}>
                              {desc.length}/90
                            </Badge>
                            {' '}{desc}
                          </p>
      ...
      <Card>
        <Card.Section>
          <Stack vertical>
            ...
            <Banner status="info">
              <p>
                Intent blocks automatically adapt your content based on how visitors arrive at your site.
                Try the preview links above to see different messaging for different audience intents.
              </p>
            </Banner>
          </Stack>
        </Card.Section>
          ...
          <Stack vertical>
            ...
            <FormLayout>
              <TextField
                ...
                onChange={(value) => setAudienceConfig(prev => ({ ...prev, listId: value }))}
              ...
              <Select
                ...
                onChange={(value) => setAudienceConfig(prev => ({ ...prev, mode: value }))}
                options={[
              ...
              {audienceConfig.mode === 'TARGET' && (
                <TextField
                  ...
                  onChange={(value) => setAudienceConfig(prev => ({ ...prev, bidModifier: value }))}
                  placeholder="1.25"
                  type="number"
                  step="0.01"
            ...
            >
              <Modal.Section>
                <TextContainer>
                  <p><strong>Step 1:</strong> Export your audience CSV from ProofKit</p>
                  ...
                  <p><strong>Step 6:</strong> Paste the ID in the field above</p>
                </TextContainer>
              </Modal.Section>
            </Modal>
          </Stack>
  ...
  function GoLiveStep() {
    ...
    const enablePromote = async () => {
      ...
      try {
        ...
        setError('Failed to go live: ' + (error instanceof Error ? error.message : 'Unknown error'));
      } finally {
        setLoading(false);
      }
    };
      ...
      <Card>
        <Card.Section>
          <Stack vertical>
            ...
            <p>Enable PROMOTE to allow live changes to your Google Ads account.</p>
            
            {!promoteEnabled && (
              <Banner status="warning">
                <p>
                  <strong>Important:</strong> Enabling PROMOTE will allow the script to make live changes 
                  to your Google Ads account based on your configuration.
                </p>
              </Banner>
            )}
            ...
            {promoteEnabled && (
              <Banner status="success">
                <p>
                  <Icon source={CheckmarkIcon} color="success" />
                  {' '}PROMOTE is now enabled! Your automation is active.
                </p>
              </Banner>
            )}
            ...
            <FormLayout>
              <TextField
                ...
                onChange={(value) => setScheduleWindow(prev => ({ ...prev, startTime: value }))}
                type="datetime-local"
              ...
              <Select
                ...
                onChange={(value) => setScheduleWindow(prev => ({ ...prev, duration: value }))}
                options={[
