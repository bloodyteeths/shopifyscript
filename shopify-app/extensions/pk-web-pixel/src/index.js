...
function getConsentStatus() {
  ...
  if (typeof google_tag_data !== "undefined" && google_tag_data.ics) {
    try {
      const analyticsConsent =
        google_tag_data.ics.get("analytics_storage") === "granted";
      ...
      const adPersonalization =
        google_tag_data.ics.get("ad_personalization") === "granted";
  ...
  const cmpChecks = [
    ...
    () =>
      ...
      window.truste.eu.bindMap.marketing,
    // Didomi
    () =>
      window.Didomi &&
      window.Didomi.getUserConsentStatusForPurpose &&
      window.Didomi.getUserConsentStatusForPurpose("marketing"),
    // GDPR Cookie Consent
    () =>
      window.wp_gdpr_cookie_consent_settings &&
      window.wp_gdpr_cookie_consent_settings.categories_accepted &&
      window.wp_gdpr_cookie_consent_settings.categories_accepted.includes(
        "marketing",
      ),
  ];
  ...
  const assumeConsentNeeded = consentRequiredRegions.some((region) =>
...
export function registerAnalytics(analytics) {
  ...
  analytics.subscribe("product_viewed", (e) => {
    ...
    const productData = {
      ...
      price:
        e?.data?.productVariant?.price?.amount || product?.price?.amount || 0,
      currency:
        e?.data?.productVariant?.price?.currencyCode ||
  ...
  analytics.subscribe("search_submitted", (e) => {
    ...
    const query =
      e?.data?.searchResult?.query || e?.data?.search?.searchTerm || "";
    const searchData = {
  ...
  analytics.subscribe("checkout_completed", (e) => {
    ...
    const purchaseData = {
      ...
      value: Number(
        checkout?.subtotalPrice?.amount || checkout?.totalPrice?.amount || 0,
      ),
      currency:
