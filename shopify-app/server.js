...
app.post("/settings", async (req, res) => {
  ...
  await fetch(
    `${BACKEND}/upsertConfig?tenant=${encodeURIComponent(TENANT)}&sig=${encodeURIComponent(sig)}`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        nonce,
        settings: { default_final_url: req.body.defaultUrl || "" },
      }),
    },
  );
...
async function proxyGet(res, path, payload) {
  ...
  const url =
    joinPath(BACKEND_BASE, path) +
    `?tenant=${encodeURIComponent(TENANT)}&sig=${encodeURIComponent(sig)}`;
...
app.post("/app/api/intent/upsert", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/intent/upsert",
    `POST:${TENANT}:intent_upsert:${nonce}`,
  );
});
app.post("/app/api/intent/delete", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/intent/delete",
    `POST:${TENANT}:intent_delete:${nonce}`,
  );
});
...
app.post("/app/api/overlays/apply", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/overlays/apply",
    `POST:${TENANT}:overlays_apply:${nonce}`,
  );
});
app.post("/app/api/overlays/revert", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/overlays/revert",
    `POST:${TENANT}:overlays_revert:${nonce}`,
  );
});
app.post("/app/api/overlays/bulk", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/overlays/bulk",
    `POST:${TENANT}:overlays_bulk:${nonce}`,
  );
});
...
app.get("/app/api/audiences/export/list", async (_req, res) => {
  await proxyGet(
    res,
    "/audiences/export/list",
    `GET:${TENANT}:audiences_export_list`,
  );
});
app.post("/app/api/audiences/export/build", async (req, res) => {
  ...
  await proxyPost(
    req,
    res,
    "/audiences/export/build",
    `POST:${TENANT}:audiences_export_build:${nonce}`,
  );
});
...
app.post("/app/api/promote/window", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/promote/window",
    `POST:${TENANT}:promote_window:${nonce}`,
  );
});
app.post("/app/api/audiences/mapUpsert", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/audiences/mapUpsert",
    `POST:${TENANT}:audiences_map_upsert:${nonce}`,
  );
});
...
app.post("/app/api/autopilot/quickstart", async (req, res) => {
  ...
  await proxyPost(
    ...
    res,
    "/autopilot/quickstart",
    `POST:${TENANT}:autopilot_quickstart:${nonce}`,
  );
});
