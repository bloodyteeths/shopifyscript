...
class AgencyBulkService {
    ...
    for (let i = 0; i < tenantIds.length; i++) {
      ...
      try {
        ...
        await this.simulateOperation(
          `Cloning template ${templateId} to ${tenantId}`,
          1000,
        );
  ...
  async processNegativeKeywords(job) {
    ...
    for (let i = 0; i < tenantIds.length; i++) {
      ...
      try {
        ...
        await this.simulateOperation(
          `Adding negative keywords for ${tenantId}`,
          900,
        );
  ...
  async processReportGeneration(job) {
    ...
    for (let i = 0; i < tenantIds.length; i++) {
      ...
      try {
        ...
        await this.simulateOperation(
          `Generating ${reportType} report for ${tenantId}`,
          2000,
        );
  ...
  async processAudienceSync(job) {
    ...
    for (let i = 0; i < tenantIds.length; i++) {
      ...
      try {
        ...
        await this.simulateOperation(
          `Syncing audience ${audienceId} for ${tenantId}`,
          1500,
        );
  ...
  async getJobs(filters = {}) {
    try {
      const jobFiles = fs
        .readdirSync(this.operationsDir)
      ...
      if (filters.createdBy) {
        filteredJobs = filteredJobs.filter(
          (job) => job.createdBy === filters.createdBy,
        );
      }
  ...
  async getBulkAnalytics(timeframe = "last_30_days") {
    try {
      ...
      const filteredJobs = jobs.filter(
        (job) => new Date(job.createdAt) >= cutoffDate,
      );
      ...
      filteredJobs.forEach((job) => {
        analytics.jobsByStatus[job.status] =
          (analytics.jobsByStatus[job.status] || 0) + 1;
        analytics.jobsByType[job.type] =
          (analytics.jobsByType[job.type] || 0) + 1;
      ...
      if (completedJobs.length > 0) {
        const totalSuccesses = completedJobs.reduce(
          (sum, job) => sum + job.successCount,
          0,
        );
        const totalOperations = completedJobs.reduce(
          (sum, job) => sum + job.successCount + job.failureCount,
          0,
        );
        analytics.averageSuccessRate =
          totalOperations > 0 ? (totalSuccesses / totalOperations) * 100 : 0;
      }
      ...
      if (jobsWithDuration.length > 0) {
        ...
        analytics.averageJobDuration = Math.round(
          totalDuration / jobsWithDuration.length / 1000,
        ); // seconds
      }
