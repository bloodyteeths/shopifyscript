...
class CacheInvalidationStrategy {
  ...
  initializeInvalidationRules() {
    ...
    this.addInvalidationRule("config:update", (tenantId, context) => {
      const patterns = [`/api/insights`, `/api/summary`, `/api/insights/terms`];
  ...
  removeDependency(dependentKey, sourceKey) {
    ...
    this.metrics.totalDependencies = Math.max(
      0,
      this.metrics.totalDependencies - 1,
    );
  }
  ...
  invalidate(tenantId, cacheKey, context = {}) {
    const fullKey = tenantCache.generateKey(
      tenantId,
      cacheKey.path,
      cacheKey.params,
    );
  ...
  invalidateByRule(pattern, tenantId, context = {}) {
    ...
    if (rule) {
      try {
        ...
        console.error(
          `Cache invalidation rule '${pattern}' failed:`,
          error.message,
        );
      }
    }
  ...
  timeBasedInvalidation(interval = "hourly") {
    const tenants = tenantCache.stats
      ? Object.keys(tenantCache.getGlobalStats())
      : [];
  ...
  analyzeDependencies() {
    ...
    for (const key of this.dependencies.keys()) {
      const hasValidCache = tenantCache.cache
        ? tenantCache.cache.has(key)
        : false;
      if (!hasValidCache) {
  ...
  cleanup() {
    ...
    this.metrics.totalDependencies = Math.max(
      0,
      this.metrics.totalDependencies - cleaned,
    );
    return cleaned;
  }
  ...
  getStats() {
    ...
    return {
      ...
      metrics: {
        ...this.metrics,
        averageCascadeDepth:
          this.metrics.dependencyInvalidations > 0
...
setInterval(
  () => {
    ...
    if (cleaned > 0) {
      console.log(
        `CacheInvalidation: Cleaned up ${cleaned} orphaned dependencies`,
      );
    }
  },
  5 * 60 * 1000,
