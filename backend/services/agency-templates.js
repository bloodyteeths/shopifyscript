...
class AgencyTemplateService {
  ...
  async getTemplates(filters = {}) {
    try {
      ...
      if (filters.publicOnly) {
        filteredTemplates = filteredTemplates.filter(t => t.metadata.isPublic);
      }
  ...
  async updateTemplate(templateId, updates) {
    try {
      ...
      if (updates.config) template.config = this.sanitizeConfiguration(updates.config);
  ...
  async bulkClone(templateId, tenantConfigs) {
    try {
      ...
      console.log(`Bulk clone completed: ${results.filter(r => r.success).length}/${results.length} successful`);
      return results;
      ...
      Object.keys(sanitized.AUDIENCE_MAP).forEach(campaign => {
        Object.keys(sanitized.AUDIENCE_MAP[campaign]).forEach(adGroup => {
          ...
          if (sanitized.AUDIENCE_MAP[campaign][adGroup].user_list_id) {
            sanitized.AUDIENCE_MAP[campaign][adGroup].user_list_id = 'PLACEHOLDER_AUDIENCE_ID';
          }
        });
      });
    }
  ...
  async importTemplate(exportData, importOptions = {}) {
    try {
      ...
      const templatePath = path.join(this.templatesDir, `${newTemplateId}.json`);
      ...
      const importedTemplate = {
        ...
        metadata: {
          ...
          original_id: exportData.id !== newTemplateId ? exportData.id : undefined
        }
      };
