/**
 * Alerts Service for Ads Autopilot AI SaaS
...
export class AlertsService {
  ...
  initializeDefaultTemplates() {
    ...
    this.templates.set("email_cost_spike", {
      type: "email",
      template: {
        subject: "🚨 Ads Autopilot AI Alert: Cost Spike Detected for {{tenant}}",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              <h1 style="color: #dc3545; margin: 0;">🚨 Cost Spike Alert</h1>
              <p style="margin: 10px 0 0 0; color: #6c757d;">Ads Autopilot AI Monitoring System</p>
            </div>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px; font-size: 12px; color: #6c757d;">
              This alert was generated by Ads Autopilot AI's automated monitoring system. 
    ...
    this.templates.set("email_weekly_summary", {
      type: "email",
      template: {
        subject: "📊 Ads Autopilot AI Weekly Summary - {{tenant}}",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #f8f9fa; padding: 15px; border-radius: 0 0 8px 8px; font-size: 12px; color: #6c757d; text-align: center;">
              Ads Autopilot AI Weekly Summary | <a href="{{unsubscribeUrl}}">Unsubscribe</a> | <a href="{{settingsUrl}}">Settings</a>
            </div>
          </div>
        `,
      },
  ...
  async sendEmailAlert(channel, alertData, options) {
    ...
    const emailPayload = {
      ...
      from: channel.fromEmail || "alerts@adsautopilot.com",
  ...
  async sendWebhookAlert(channel, alertData, options) {
    ...
    const response = await fetch(channel.url, {
      ...
      headers: {
        "Content-Type": "application/json",
        "User-Agent": "Ads Autopilot AI-Alerts/1.0",
        ...(channel.headers || {}),
      },
  ...
  renderTemplate(template, alertData, options) {
    const data = {
      ...
      dashboardUrl:
        ...
        `https://app.adsautopilot.com/dashboard/${alertData.tenant}`,
      suppressUrl:
        options.suppressUrl ||
        `https://app.adsautopilot.com/alerts/suppress/${alertData.tenant}/${alertData.type}`,
      settingsUrl:
        options.settingsUrl ||
        `https://app.adsautopilot.com/settings/${alertData.tenant}`,
      unsubscribeUrl:
        options.unsubscribeUrl ||
        `https://app.adsautopilot.com/unsubscribe/${alertData.tenant}`,
