...
export class AlertsService {
  ...
  registerChannel(tenant, channelId, config) {
    ...
    logger.info('Alert channel registered', { tenant, channelId, type: config.type });
  }
  ...
  async sendAlert(tenant, alertData, options = {}) {
    ...
    logger.info('Sending alert', { tenant, alertType: alertData.type, severity: alertData.severity });
    ...
    try {
      ...
      if (this.isThrottled(tenant, alertData.type, alertData.severity)) {
        logger.info('Alert throttled', { tenant, alertType: alertData.type, severity: alertData.severity });
      ...
      for (const [channelId, channel] of tenantChannels) {
        if (channel.enabled && this.shouldSendToChannel(channel, alertData, options)) {
          promises.push(
            this.sendToChannel(channel, alertData, options)
              .then(result => ({ channelId, success: true, result }))
              .catch(error => ({ channelId, success: false, error: error.message }))
          );
        }
      }
  ...
  async sendSlackAlert(channel, alertData, options) {
    ...
    if (!template) {
      throw new Error(`No Slack template found for alert type: ${alertData.type}`);
    }
    ...
    return { platform: 'slack', messageId: response.headers.get('x-slack-req-id') };
  }
  ...
  async sendEmailAlert(channel, alertData, options) {
    ...
    if (!template) {
      throw new Error(`No email template found for alert type: ${alertData.type}`);
    }
  ...
  renderTemplate(template, alertData, options) {
    const data = {
      ...
      dashboardUrl: options.dashboardUrl || `https://app.proofkit.com/dashboard/${alertData.tenant}`,
      suppressUrl: options.suppressUrl || `https://app.proofkit.com/alerts/suppress/${alertData.tenant}/${alertData.type}`,
      settingsUrl: options.settingsUrl || `https://app.proofkit.com/settings/${alertData.tenant}`,
      unsubscribeUrl: options.unsubscribeUrl || `https://app.proofkit.com/unsubscribe/${alertData.tenant}`,
  ...
  async sendWeeklySummary(tenant, summaryData, options = {}) {
    ...
    const templateData = {
      ...
      cpaTrend: summaryData.trends.cpa ? this.formatTrend(summaryData.trends.cpa) : 'N/A',
      ...
      cpaTrendColor: summaryData.trends.cpa ? this.getTrendColor(summaryData.trends.cpa, true) : '#6c757d',
      aiInsights: summaryData.aiInsights?.summary || 'No AI insights available',
      topSearchTerms: summaryData.searchTerms.slice(0, 5)
        .map(term => `<li>"${term.term}" - ${term.conversions} conversions</li>`)
        .join('')
    };
  ...
  getDeliveryStats(tenant) {
    ...
    const totalSuccessful = recent.reduce((sum, delivery) => sum + delivery.channelsSuccessful, 0);
    const totalAttempted = recent.reduce((sum, delivery) => sum + delivery.channelsAttempted, 0);
  ...
  async testChannel(tenant, channelId) {
    ...
    try {
      const result = await this.sendToChannel(channel, testAlert, {
        templateData: { message: 'Test Alert - Channel Configuration Verification' }
      });
      ...
      logger.error('Channel test failed', { tenant, channelId, error: error.message });
