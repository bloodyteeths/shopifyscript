...
class BootValidationService {
  ...
  async validateSystemServices() {
    ...
    try {
      ...
      try {
        ...
        logger.error(
          "🚨 CRITICAL SECURITY ISSUE: HMAC secret validation failed",
          {
            error: error.message,
  ...
  async validateTenant(tenantId, options = {}) {
    ...
    try {
      ...
      for (const [sheetName, schema] of schemas) {
        try {
          const sheetResult = await this.validateSheetSchema(
            tenantId,
            sheetName,
            schema,
            options,
  ...
  async validateSheetSchema(tenantId, sheetName, schema, options = {}) {
    ...
    try {
      ...
      const sheetInfo = await sheetsSchema.ensureSheetWithSchema(
        tenantId,
        sheetName,
      );
      ...
      if (
        ...
        sheetInfo.columnCount === schema.headers.length
      ) {
        sheetResult.created = true;
      }

      // Validate schema
      const validation = await sheetsSchema.validateSheetSchema(
        tenantId,
        sheetName,
        schema,
      );
  ...
  async createEmergencyBackups(validationResults) {
    ...
    logger.info(
      `Creating emergency backups for ${tenantsWithErrors.length} tenants with issues`,
    );
    ...
    const backupPromises = tenantsWithErrors.map(async (tenantId) => {
      try {
        ...
        logger.info(`Emergency backup queued for tenant: ${tenantId}`, {
          backupId,
        });
        return { tenantId, backupId, success: true };
      } catch (error) {
        logger.error(
          `Failed to queue emergency backup for tenant: ${tenantId}`,
          {
            error: error.message,
          },
        );
  ...
  updateMetrics(results, validationTime) {
    ...
    this.metrics.avgValidationTime =
      (this.metrics.avgValidationTime * (this.metrics.validationsRun - 1) +
        validationTime) /
      this.metrics.validationsRun;
  }
  ...
  async healthCheck() {
    ...
    if (
      this.lastValidation &&
      this.lastValidation.overallStatus === "unhealthy"
    ) {
