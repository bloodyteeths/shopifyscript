...
class AudienceAnalyticsService {
    ...
    this.analyticsQueries = {
      customerSegmentation: {
        ...
        queries: {
          ...
          mediumValue:
            "SELECT * FROM customers WHERE total_spent >= 100 AND total_spent < 500",
      ...
      purchaseBehavior: {
        ...
        queries: {
          recentBuyers:
            "SELECT * FROM customers WHERE last_order_at >= '2024-01-01'",
          categoryLovers:
            "SELECT top_category, COUNT(*) as customers FROM customers GROUP BY top_category ORDER BY customers DESC",
          spendingTrends:
            "SELECT total_spent, COUNT(*) as customers FROM customers GROUP BY total_spent ORDER BY total_spent",
        },
      },
      audienceHealth: {
        ...
        queries: {
          ...
          avgOrderValue:
            "SELECT AVG(total_spent / order_count) as aov FROM customers WHERE order_count > 0",
          totalRevenue: "SELECT SUM(total_spent) as revenue FROM customers",
          avgOrdersPerCustomer:
            "SELECT AVG(order_count) as avg_orders FROM customers",
        },
      },
    };
  }
  ...
  async generateAudienceAnalytics(tenantId, options = {}) {
    ...
    try {
      ...
      if (includeHealth) {
        analytics.audienceHealth = await this.calculateAudienceHealth(
          tenantId,
          customers,
        );
      }
      ...
      if (includeSegmentation) {
        analytics.segmentation = await this.calculateSegmentation(
          tenantId,
          customers,
        );
      }
      ...
      if (includeBehavior) {
        analytics.behavior = await this.calculateBehaviorMetrics(
          tenantId,
          customers,
        );
      }
      ...
      console.log(
        `Audience analytics completed for tenant ${tenantId} in ${executionTime}ms`,
      );
      return analytics;
    } catch (error) {
  ...
  segmentByValue(customers) {
    const segments = {
      ...
      high: customers.filter(
        (c) =>
          Number(c.total_spent || 0) >= 500 &&
          Number(c.total_spent || 0) < 1000,
      ),
      medium: customers.filter(
        (c) =>
          Number(c.total_spent || 0) >= 100 && Number(c.total_spent || 0) < 500,
      ),
    ...
    return {
      vip: {
        ...
        percentage: ((segments.vip.length / customers.length) * 100).toFixed(1),
      },
      high: {
        count: segments.high.length,
        percentage: ((segments.high.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      medium: {
        count: segments.medium.length,
        percentage: ((segments.medium.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      low: {
        count: segments.low.length,
        percentage: ((segments.low.length / customers.length) * 100).toFixed(1),
      },
    };
  }
  ...
  segmentByFrequency(customers) {
    const segments = {
      ...
      repeat: customers.filter(
        (c) =>
          Number(c.order_count || 0) >= 3 && Number(c.order_count || 0) < 10,
      ),
    ...
    return {
      loyal: {
        ...
        percentage: ((segments.loyal.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      repeat: {
        count: segments.repeat.length,
        percentage: ((segments.repeat.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      occasional: {
        ...
        percentage: (
          (segments.occasional.length / customers.length) *
          100
        ).toFixed(1),
      },
      oneTime: {
        ...
        percentage: (
          (segments.oneTime.length / customers.length) *
          100
        ).toFixed(1),
      },
    };
  ...
  segmentByRecency(customers) {
    ...
    const segments = {
      recent: customers.filter(
        (c) => new Date(c.last_order_at || 0) >= thirtyDaysAgo,
      ),
      ...
      churned: customers.filter(
        (c) => new Date(c.last_order_at || 0) < oneYearAgo,
      ),
    };

    return {
      recent: {
        count: segments.recent.length,
        percentage: ((segments.recent.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      active: {
        count: segments.active.length,
        percentage: ((segments.active.length / customers.length) * 100).toFixed(
          1,
        ),
      },
      dormant: {
        ...
        percentage: (
          (segments.dormant.length / customers.length) *
          100
        ).toFixed(1),
      },
      churned: {
        ...
        percentage: (
          (segments.churned.length / customers.length) *
          100
        ).toFixed(1),
      },
    };
  ...
  calculateRFMSegmentation(customers) {
    ...
    const customersWithRFM = customers.map((customer) => {
      ...
      const frequency = this.calculateFrequencyScore(
        Number(customer.order_count || 0),
      );
      const monetary = this.calculateMonetaryScore(
    ...
    const segments = {
      champions: customersWithRFM.filter(
        (c) =>
          c.rfm.recency >= 4 && c.rfm.frequency >= 4 && c.rfm.monetary >= 4,
      ),
      loyalCustomers: customersWithRFM.filter(
        (c) =>
          c.rfm.recency >= 3 && c.rfm.frequency >= 3 && c.rfm.monetary >= 3,
      ),
      potentialLoyalists: customersWithRFM.filter(
        (c) =>
          c.rfm.recency >= 3 && c.rfm.frequency <= 2 && c.rfm.monetary >= 2,
      ),
      newCustomers: customersWithRFM.filter(
        (c) =>
          c.rfm.recency >= 4 && c.rfm.frequency <= 2 && c.rfm.monetary <= 2,
      ),
      promising: customersWithRFM.filter(
        (c) =>
          c.rfm.recency >= 3 && c.rfm.frequency <= 2 && c.rfm.monetary <= 2,
      ),
      needAttention: customersWithRFM.filter(
        (c) =>
          c.rfm.recency <= 2 && c.rfm.frequency >= 3 && c.rfm.monetary >= 3,
      ),
      aboutToSleep: customersWithRFM.filter(
        (c) =>
          c.rfm.recency <= 2 && c.rfm.frequency <= 2 && c.rfm.monetary >= 2,
      ),
      atRisk: customersWithRFM.filter(
        (c) =>
          c.rfm.recency <= 2 && c.rfm.frequency >= 2 && c.rfm.monetary <= 2,
      ),
      cannotLoseThem: customersWithRFM.filter(
        (c) =>
          c.rfm.recency <= 2 && c.rfm.frequency >= 4 && c.rfm.monetary >= 4,
      ),
      hibernating: customersWithRFM.filter(
        (c) =>
          c.rfm.recency <= 2 && c.rfm.frequency <= 2 && c.rfm.monetary <= 2,
      ),
    };
  ...
  calculateValueSegments(customers) {
    ...
    Object.keys(segments).forEach((segment) => {
      ...
      segments[segment].revenueContribution =
        totalRevenue > 0
          ? ((segmentRevenue / totalRevenue) * 100).toFixed(1)
          : "0";
      segments[segment].avgSpending =
        segmentCustomers.length > 0
          ? (segmentRevenue / segmentCustomers.length).toFixed(2)
          : "0";
    });
  ...
  calculateRetentionRates(customers) {
    ...
    const repeatCustomers = customers.filter(
      (c) => Number(c.order_count || 0) > 1,
    ).length;
    const loyalCustomers = customers.filter(
      (c) => Number(c.order_count || 0) >= 5,
    ).length;
    ...
    return {
      ...
      loyaltyRate:
        totalCustomers > 0
          ? ((loyalCustomers / totalCustomers) * 100).toFixed(1)
          : "0",
    };
  }
    ...
    if (analytics.behavior && analytics.behavior.churnRisk) {
      ...
      if (Number(churn.atRisk.percentage) > 10) {
        recommendations.push({
          ...
          action:
            "Create a re-engagement campaign targeting customers who haven't purchased in 90+ days",
        });
      }
    }
    ...
    if (analytics.segmentation && analytics.segmentation.byFrequency) {
      ...
      if (Number(freq.oneTime.percentage) > 50) {
        recommendations.push({
          ...
          action:
            "Implement a follow-up email series to encourage second purchases",
        });
      }
    }

    if (
      analytics.audienceHealth &&
      analytics.audienceHealth.averageOrderValue < 50
    ) {
      recommendations.push({
        type: "revenue",
        priority: "medium",
        ...
        action:
          "Consider product bundling, upsells, or minimum order incentives",
      });
    }
  ...
  async getSegmentPerformance(tenantId, segmentKey, timeframe = "30d") {
    try {
      ...
      const performance = {
        ...
        totalRevenue: segmentCustomers.reduce(
          (sum, c) => sum + Number(c.total_spent || 0),
          0,
        ),
        averageOrderValue: 0,
      ...
      if (segmentCustomers.length > 0) {
        const totalOrders = segmentCustomers.reduce(
          (sum, c) => sum + Number(c.order_count || 0),
          0,
        );
        performance.averageOrderValue =
          totalOrders > 0 ? performance.totalRevenue / totalOrders : 0;

        const repeatCustomers = segmentCustomers.filter(
          (c) => Number(c.order_count || 0) > 1,
        ).length;
        performance.retentionRate =
          (repeatCustomers / segmentCustomers.length) * 100;
      }
  ...
  generateSegmentComparison(segments) {
    ...
    const bestRevenue = segments.reduce((max, s) =>
      s.totalRevenue > max.totalRevenue ? s : max,
    );
    const bestAOV = segments.reduce((max, s) =>
      s.averageOrderValue > max.averageOrderValue ? s : max,
    );
    const bestRetention = segments.reduce((max, s) =>
      s.retentionRate > max.retentionRate ? s : max,
    );
  ...
  updateMetrics(startTime) {
    ...
    this.metrics.avgCalculationTime =
      (this.metrics.avgCalculationTime + executionTime) /
      this.metrics.analyticsGenerated;
  }
