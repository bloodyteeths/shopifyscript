...
export class UpgradeFlowService {
  ...
  generateUpgradePrompt(feature, userTier, platform = 'stripe') {
    ...
    const requiredTierIndex = this.billingService.getTierIndexById(requiredTier);
   ...
   */
  generateUsageLimitWarning(limitType, currentUsage, userTier, platform = 'stripe') {
    ...
    const nextTier = Object.values(this.billingService.PRICING_TIERS)[nextTierIndex];
    ...
    return {
      ...
      warningLevel: usagePercentage >= 95 ? 'critical' : usagePercentage >= 90 ? 'high' : 'medium',
      ...
      message: this.getUsageLimitMessage(limitType, usagePercentage, nextTier.name)
    };
  }
  ...
  generateOnboardingUpgrade(userBehavior, platform = 'stripe') {
    ...
    if (featuresAttempted.includes('ai_copywriter') || featuresAttempted.includes('rsa_test_queue')) {
      recommendedTierIndex = 1; // Pro
    }
    
    if (featuresAttempted.includes('asset_library') || timeSpent > 30 * 60 * 1000) { // 30+ minutes
    ...
    const recommendedTier = Object.values(this.billingService.PRICING_TIERS)[recommendedTierIndex];
    ...
    return {
      ...
      personalizedBenefits: this.getPersonalizedBenefits(featuresAttempted, recommendedTier),
  ...
  async handlePostUpgrade(userId, newTierIndex, platform) {
    try {
      const newTier = Object.values(this.billingService.PRICING_TIERS)[newTierIndex];
  ...
  generateSmartUpgradeSuggestions(userId, usageData, platform = 'stripe') {
    ...
    if (campaigns > 15 && usageData.currentTier === 'starter') {
      suggestions.push({
        ...
        description: 'Pro tier gives you 4x the campaign limit plus AI copywriting',
        tierIndex: 1,
        priority: 'high'
      });
    }
    ...
    if (monthlySpend > 10000 && usageData.currentTier !== 'enterprise') {
      suggestions.push({
        ...
        description: 'Growth tier includes advanced budget optimization for high-spend accounts',
        tierIndex: 2,
        priority: 'high'
      });
    }
    ...
    return suggestions.map(suggestion => ({
      ...
      tier: Object.values(this.billingService.PRICING_TIERS)[suggestion.tierIndex]
    }));
  }
  ...
  getUpgradeBenefits(currentTierIndex, targetTierIndex) {
    ...
    for (let i = currentTierIndex + 1; i <= targetTierIndex; i++) {
      ...
      if (tier) {
        benefits.push(...tier.features.filter(f => !f.startsWith('Everything in')));
      }
    }
  ...
  getUpgradeUrl(tierIndex, platform) {
    const baseUrl = platform === 'shopify' ? '/billing' : '/wp-admin/admin.php?page=proofkit-billing';
  ...
  getUsageLimitMessage(limitType, usagePercentage, nextTier) {
    ...
    return limitMessages[limitType] || `You're at ${usagePercentage}% capacity. Upgrade to ${nextTier} for more resources.`;
  }
  ...
  getSocialProof(tierId) {
    const proofData = {
      ...
      pro: { customers: 800, testimonial: 'AI copywriting saves us hours every week' },
      growth: { customers: 300, testimonial: 'Scaled from $10k to $100k monthly spend' },
      enterprise: { customers: 50, testimonial: 'Custom rules give us complete control' }
    };
  ...
  async sendUpgradeWelcomeEmail(userId, tier, platform) {
    ...
    logger.info('Upgrade welcome email scheduled', { userId, tier: tier.id, platform });
  }

  async enableTierFeatures(userId, features, platform) {
    // Implementation would update user permissions/features
    logger.info('Tier features enabled', { userId, features: features.length, platform });
  }
