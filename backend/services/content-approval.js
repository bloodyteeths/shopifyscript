...
export class ContentApprovalWorkflow {
  ...
  async submitForApproval(content, options = {}) {
    ...
    try {
      ...
      const submission = {
        ...
        autoQualityCheck: await this.performAutoQualityCheck(
          content,
          contentType,
        ),
  ...
  async performAutoQualityCheck(content, contentType) {
    ...
    try {
      ...
      checks.lengthCompliance = this.validateContentLength(
        content,
        contentType,
      );
  ...
  validateContentFormat(content, contentType) {
    switch (contentType) {
      case CONTENT_TYPES.RSA_HEADLINES:
        return (
          Array.isArray(content) && content.every((h) => typeof h === "string")
        );
      case CONTENT_TYPES.RSA_DESCRIPTIONS:
        return (
          Array.isArray(content) && content.every((d) => typeof d === "string")
        );
      case CONTENT_TYPES.SITELINKS:
        return (
          Array.isArray(content) &&
          content.every(
            (s) => s.text && s.final_url && typeof s.text === "string",
          )
        );
      ...
      case CONTENT_TYPES.NEGATIVE_KEYWORDS:
        return (
          Array.isArray(content) &&
          content.every((k) => k.keyword && typeof k.keyword === "string")
        );
      default:
...
Return only "PASS" or "FAIL" based on whether the content meets professional advertising standards.`;
  ...
  getSubmission(submissionId) {
    return (
      this.approvalQueue.get(submissionId) ||
      this.reviewHistory.get(submissionId)
    );
  }
  ...
  getWorkflowStats(tenant = null) {
    ...
    for (const submission of this.approvalQueue.values()) {
      ...
      if (submission.status === APPROVAL_STATES.PENDING_REVIEW)
        stats.pendingReview++;
      if (submission.status === APPROVAL_STATES.NEEDS_REVISION)
        stats.needsRevision++;
    }
    ...
    for (const submission of this.reviewHistory.values()) {
      ...
      if (submission.status === APPROVAL_STATES.APPROVED) {
        ...
        const approvalAction = submission.history.find(
          (h) => h.action === "approved",
        );
