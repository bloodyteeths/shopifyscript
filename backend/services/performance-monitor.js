...
class PerformanceMonitor {
  constructor() {
    this.config = {
      targetResponseTime: Number(
        process.env.PERFORMANCE_TARGET_RESPONSE_MS || 200,
      ),
      targetCacheHitRate: Number(
        process.env.PERFORMANCE_TARGET_CACHE_HIT_RATE || 80,
      ),
      monitoringInterval: Number(
        process.env.PERFORMANCE_MONITORING_INTERVAL || 30000,
      ), // 30 seconds
      alertThreshold: Number(process.env.PERFORMANCE_ALERT_THRESHOLD || 3), // 3 violations before alert
      reportingInterval: Number(
        process.env.PERFORMANCE_REPORTING_INTERVAL || 300000,
      ), // 5 minutes
      metricsRetention: Number(
        process.env.PERFORMANCE_METRICS_RETENTION || 86400000,
      ), // 24 hours
    };
  ...
  updateResponseTimeMetrics(responseMetrics, timestamp) {
    ...
    const recentSamples = this.getRecentSamples(
      this.metrics.responseTime.samples,
      3600000,
    ); // Last hour
    if (recentSamples.length > 0) {
      const sortedTimes = recentSamples
        .map((s) => s.value)
        .sort((a, b) => a - b);
    ...
    const samples24h = this.getRecentSamples(
      this.metrics.responseTime.samples,
      86400000,
    );
    if (samples24h.length > 0) {
      this.metrics.responseTime.avg24h =
        samples24h.reduce((sum, s) => sum + s.value, 0) / samples24h.length;
    }
  }
  ...
  updateCacheMetrics(cacheMetrics, timestamp) {
    ...
    // Calculate 24h average
    const samples24h = this.getRecentSamples(
      this.metrics.cacheHitRate.samples,
      86400000,
    );
    if (samples24h.length > 0) {
      this.metrics.cacheHitRate.avg24h =
        samples24h.reduce((sum, s) => sum + s.value, 0) / samples24h.length;
    }
  }
  ...
  updateOptimizationMetrics(
    ...
    if (responseMetrics.totalCompressionSavings) {
      const savingsKB =
        parseFloat(
          responseMetrics.totalCompressionSavings.replace(" KB", ""),
        ) || 0;
      this.metrics.optimization.compressionSavings = savingsKB;
    }

    // Cache warming hits
    this.metrics.optimization.cacheWarmingHits =
      optimizerMetrics.warmingHits || 0;

    // Connection pool utilization
    if (sheetsMetrics.totalConnections > 0) {
      this.metrics.optimization.connectionPoolUtilization =
        (sheetsMetrics.activeConnections / sheetsMetrics.totalConnections) *
        100;
    }
  }
  ...
  updateThroughputMetrics(responseMetrics, timestamp) {
    ...
    const recentSamples = this.getRecentSamples(
      this.metrics.throughput.samples,
      60000,
    ); // Last minute
  ...
  validateTargets() {
    ...
    if (this.metrics.responseTime.current > this.config.targetResponseTime) {
      ...
      violations.push({
        ...
        severity: this.calculateSeverity(
          this.metrics.responseTime.current,
    ...
    if (this.metrics.cacheHitRate.current < this.config.targetCacheHitRate) {
      ...
      violations.push({
        ...
        severity: this.calculateCacheHitSeverity(
          this.metrics.cacheHitRate.current,
          this.config.targetCacheHitRate,
        ),
      });
    }
  ...
  handleViolations(violations) {
    ...
    this.alerts = this.alerts.filter(
      (alert) => Date.now() - alert.timestamp < this.config.metricsRetention,
    );
  }
  ...
  generateRecommendations(violation) {
    ...
    if (violation.type === "RESPONSE_TIME") {
      ...
      recommendations.push(
        "Increase cache TTL values for frequently accessed data",
      );
    ...
    const status = {
      ...
      targets: {
        responseTime: {
          ...
          status:
            this.metrics.responseTime.current <= this.config.targetResponseTime
              ? "PASS"
              : "FAIL",
        ...
        cacheHitRate: {
          ...
          status:
            this.metrics.cacheHitRate.current >= this.config.targetCacheHitRate
              ? "PASS"
              : "FAIL",
      ...
      optimization: {
        compressionSavings:
          this.metrics.optimization.compressionSavings.toFixed(1) + " KB",
        cacheWarmingHits: this.metrics.optimization.cacheWarmingHits,
        connectionPoolUtilization:
          this.metrics.optimization.connectionPoolUtilization.toFixed(1) + "%",
      },
  ...
  generatePerformanceReport() {
    ...
    this.performanceReports = this.performanceReports.filter(
      (r) => Date.now() - r.timestamp < this.config.metricsRetention,
    );
  ...
  calculateCompliance(metric) {
    ...
    for (const sample of recentSamples) {
      if (
        metric === "responseTime" &&
        sample.value <= this.config.targetResponseTime
      ) {
        compliantSamples++;
      } else if (
        metric === "cacheHitRate" &&
        sample.value >= this.config.targetCacheHitRate
      ) {
        compliantSamples++;
      }
    }
  ...
  calculateUptimePercentage() {
    ...
    const healthySamples = recentSamples.filter(
      (s) => s.value > 0 && s.value < 10000,
    ); // Exclude timeouts
  ...
  calculateAvgThroughput() {
    ...
    const timeSpan =
      ...
      1000;
    const requestDiff =
      recentSamples[recentSamples.length - 1].requests -
      recentSamples[0].requests;
  ...
  generateGlobalRecommendations() {
    ...
    if (this.metrics.responseTime.avg24h > this.config.targetResponseTime) {
      recommendations.push(
        "Response time consistently above target - consider infrastructure scaling",
      );
    }
    ...
    if (this.metrics.throughput.requestsPerSecond > 50) {
      recommendations.push(
        "High traffic detected - monitor resource utilization",
      );
    }
  ...
  cleanupOldMetrics() {
    ...
    this.metrics.responseTime.samples =
      this.metrics.responseTime.samples.filter((s) => s.timestamp > cutoff);
    this.metrics.cacheHitRate.samples =
      this.metrics.cacheHitRate.samples.filter((s) => s.timestamp > cutoff);
