...
export class AnomalyDetectionService {
  ...
  async detectAnomalies(tenant, timeWindow = "24h") {
    ...
    try {
      ...
      anomalies.warnings = this.filterSuppressedAlerts(
        anomalies.warnings,
        tenant,
      );
  ...
  async fetchAnalysisData(doc, tenant, timeWindow) {
    ...
    const historicalData = this.processMetricsData(
      rows,
      historicalStartTime,
      startTime,
    );
  ...
  async detectStatisticalAnomalies(data, tenant) {
    ...
    if (data.current.totals.cost > 0) {
      ...
      const costZScore = this.calculateZScore(
        data.current.totals.cost,
        stats.cost.mean,
        stats.cost.stdDev,
      );
      ...
      if (data.current.totals.cpa && stats.cpa.count > 0) {
        const cpaZScore = this.calculateZScore(
          data.current.totals.cpa,
          stats.cpa.mean,
          stats.cpa.stdDev,
        );
        if (cpaZScore > 2) {
          // Only alert on high CPA, not low
          anomalies.alerts.push({
      ...
      const conversionRate =
        (data.current.totals.conversions /
          Math.max(data.current.totals.clicks, 1)) *
        100;
      if (stats.conversionRate.count > 0) {
        const crZScore = this.calculateZScore(
          conversionRate,
          stats.conversionRate.mean,
          stats.conversionRate.stdDev,
        );
        if (crZScore < -2) {
          // Only alert on low conversion rates
  ...
  async detectThresholdAnomalies(data, tenant) {
    ...
    if (
      data.current.totals.conversions === 0 &&
      data.current.totals.cost > 50
    ) {
      const hoursWithoutConversions = this.calculateHoursWithoutConversions(
        data.hourly,
      );
    ...
    const conversionRate =
      ...
      100;
    if (
      data.current.totals.clicks > 100 &&
      conversionRate < thresholds.low_conversion_rate_percent
    ) {
      anomalies.warnings.push({
  ...
  async detectPatternAnomalies(data, tenant) {
    ...
    const campaignDivergence = this.detectCampaignDivergence(
      data.current.campaigns,
    );
  ...
  processMetricsData(rows, startTime, endTime) {
    ...
    rows.forEach((row) => {
      ...
      if (!isFinite(rowDate) || rowDate < startTime || rowDate >= endTime)
        return;
    ...
    const campaignsArray = Array.from(campaigns.entries()).map(
      ([name, data]) => ({
        name,
        ...data,
      ...
      if (metric === "conversionRate") {
        historicalData.forEach((period) => {
          if (period.totals.clicks > 0) {
            values.push(
              (period.totals.conversions / period.totals.clicks) * 100,
            );
          }
        });
      } else {
        historicalData.forEach((period) => {
          if (
            period.totals[metric] !== null &&
            period.totals[metric] !== undefined
          ) {
  ...
  analyzeSpendingPattern(hourlyData) {
    ...
    const anomalousHours = hourlyData.filter(
      (hour) =>
  ...
  detectHourlyAnomalies(hourlyData) {
    ...
    hourlyAverages.forEach((data, hour) => {
      if (data.cost.length > 1) {
        const avgCost =
          data.cost.reduce((sum, val) => sum + val, 0) / data.cost.length;
        const maxCost = Math.max(...data.cost);

        if (maxCost > avgCost * 3) {
          // 300% above average for that hour
          anomalies.push({
            hour,
            metric: "cost",
  ...
  getAnomalySummary(tenant) {
    ...
    const summary = {
      ...
      totalWarnings: recentHistory.reduce(
        (sum, run) => sum + run.warningCount,
        0,
      ),
      thresholds: this.getThresholds(tenant),
      suppressions: Array.from(
        this.suppressionPeriods.get(tenant)?.entries() || [],
