...
class HealthService {
  ...
  async runCheck(name) {
    ...
    while (attempt <= checkConfig.retries) {
      try {
        ...
        if (attempt <= checkConfig.retries) {
          console.warn(`Health check '${name}' failed (attempt ${attempt}), retrying...`);
    ...
    const checkResult = {
      name,
      status: checkConfig.critical ? HealthStatus.CRITICAL : HealthStatus.UNHEALTHY,
  ...
  async getReadiness() {
    ...
    const failed = results.filter(r => 
      r.status === HealthStatus.UNHEALTHY || r.status === HealthStatus.CRITICAL
    );
  ...
  registerDefaultChecks() {
    ...
    this.registerCheck('memory', async () => {
      ...
      return {
        ...
        data: { heapUsedMB: Math.round(heapUsedMB), heapTotalMB: Math.round(heapTotalMB) }
      };
  ...
  registerSheetsCheck(sheetsService) {
    this.registerCheck('googleSheets', async () => {
      try {
  ...
  registerGeminiCheck(aiService) {
    this.registerCheck('geminiAI', async () => {
      try {
