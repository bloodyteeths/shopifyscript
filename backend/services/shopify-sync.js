...
class ShopifyDataIngestionService {
  ...
  transformCustomerData(customer) {
    ...
    const emailHash = customer.email ? this.hashPII(customer.email, 'email') : null;
    const phoneHash = customer.phone ? this.hashPII(customer.phone, 'phone') : null;
  ...
  async syncCustomers(tenantId, customers, options = {}) {
    ...
    try {
      ...
      console.log(`Starting Shopify sync for tenant ${tenantId}: ${customers.length} customers`);
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          console.log(`Processed batch ${batchIndex + 1}/${batches.length}: ${batch.length} customers`);
      ...
      // Update metrics
      this.updateMetrics(startTime, transformedCustomers.length, processedCount, skippedCount, errors.length);
  ...
  async syncProducts(tenantId, products, options = {}) {
    ...
    try {
        ...
        try {
          ...
          if (product.variants && Array.isArray(product.variants)) {
            for (const variant of product.variants) {
              ...
              const margin = price > 0 && cost > 0 ? ((price - cost) / price * 100).toFixed(2) : '0';
      ...
      // Sync margin data
      const marginData = transformedProducts.map(p => ({ sku: p.sku, margin: p.margin }));
      const stockData = transformedProducts.map(p => ({ sku: p.sku, stock: p.stock }));
  ...
  getSyncStatus(tenantId) {
    ...
    if (activeSync) {
      return {
        ...
        progress: activeSync.totalRecords > 0 
          ? Math.round((activeSync.processed || 0) / activeSync.totalRecords * 100)
          : 0
      };
    }
  ...
  updateMetrics(startTime, total, processed, skipped, errors) {
    ...
    this.metrics.avgProcessingTime = avgTime === 0 
      ? processingTime 
      : (avgTime + processingTime) / 2;
  }
