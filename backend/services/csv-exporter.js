...
class CSVExportService {
  constructor() {
    this.exportFormats = {
      ...
      GOOGLE_ADS_API: {
        name: 'Google Ads API',
        headers: ['hashedEmail', 'hashedPhoneNumber', 'hashedFirstName', 'hashedLastName', 'countryCode', 'postalCode'],
      ...
      GENERIC_CSV: {
        name: 'Generic CSV',
        headers: ['customer_id', 'email_hash', 'phone_hash', 'total_spent', 'order_count'],
  ...
  async generateExport(tenantId, options = {}) {
    ...
    try {
      ...
      console.log(`Starting CSV export for tenant ${tenantId}, segment ${segmentKey}, format ${format}`);
      ...
      if (segmentKey === 'all_customers') {
        segmentData = await this.loadAllCustomers(tenantId, filters, maxRows || formatConfig.maxRows);
      } else {
        segmentData = await this.loadSegmentData(tenantId, segmentKey, maxRows || formatConfig.maxRows);
      }
      ...
      const transformedData = this.transformDataForFormat(segmentData, format, formatConfig);
      ...
      // Generate file name
      const finalFileName = fileName || this.generateFileName(tenantId, segmentKey, format);
  ...
  async loadSegmentData(tenantId, segmentKey, maxRows = 100000) {
    try {
      ...
      const result = await segmentEngine.executeSegment(tenantId, segmentKey, segment.logic, {
        maxRows,
        useCache: true
      });
  ...
  applyFilters(data, filters) {
    ...
    return data.filter(customer => {
      // Min spend filter
      if (filters.minSpend && Number(customer.total_spent || 0) < Number(filters.minSpend)) {
        return false;
      }

      // Max spend filter
      if (filters.maxSpend && Number(customer.total_spent || 0) > Number(filters.maxSpend)) {
        return false;
      }

      // Min orders filter
      if (filters.minOrders && Number(customer.order_count || 0) < Number(filters.minOrders)) {
        return false;
      }
  ...
  transformForGoogleAdsUI(data, config) {
    return data.map(customer => {
      ...
      const email = customer.email || this.unhashEmail(customer.email_hash) || '';
      const phone = customer.phone || this.unhashPhone(customer.phone_hash) || '';
  ...
  transformForGoogleAdsAPI(data, config) {
    return data.map(customer => {
      return {
        hashedEmail: customer.email_hash || this.hashForGoogleAds(customer.email),
        hashedPhoneNumber: customer.phone_hash || this.hashForGoogleAds(this.formatPhoneForGoogleAds(customer.phone)),
  ...
  validateRequiredFields(data, formatConfig) {
    ...
    const sample = data[0];
    const missingFields = formatConfig.requiredFields.filter(field => 
      !Object.keys(sample).includes(field)
    );
    ...
    if (emptyRequiredFields.length > 0) {
      throw new Error(`Required fields have no values: ${emptyRequiredFields.join(', ')}`);
    }
  }
  ...
  async getExportHistory(tenantId, options = {}) {
    ...
    try {
      const exports = await optimizedSheets.getRows(tenantId, 'AUDIENCE_EXPORT', {
        limit,
      ...
      if (segmentKey) {
        filteredExports = exports.filter(exp => exp.segment_key === segmentKey);
      }
      ...
      console.error(`Failed to get export history for tenant ${tenantId}:`, error);
      return [];
    }
  }
  ...
  updateMetrics(startTime, rowCount) {
    ...
    this.metrics.avgExportTime = avgTime === 0 
      ? executionTime 
      : (avgTime + executionTime) / 2;
  }
