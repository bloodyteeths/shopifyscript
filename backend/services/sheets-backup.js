...
class SheetsBackupService {
  ...
  async createFullBackup(tenantId, options = {}) {
    ...
    try {
      ...
      try {
        ...
        logger.error('Failed to store backup metadata', { error: metaError.message });
      }
  ...
  async restoreFromBackup(tenantId, backupId, options = {}) {
    try {
      ...
      // Get backup metadata
      const backupRows = await optimizedSheets.getRows(tenantId, '_proofkit_backups');
      ...
      if (backupRecord.status !== 'completed') {
        throw new Error(`Backup is not in completed state: ${backupRecord.status}`);
      }
      ...
      for (const [sheetName, schemaData] of Object.entries(backupData.schemas)) {
        try {
      ...
      await optimizedSheets.updateRow(tenantId, '_proofkit_backups', backupRecord);
  ...
  async listBackups(tenantId, options = {}) {
    try {
      ...
      let backups = backupRows.filter(row => 
  ...
  async deleteBackup(tenantId, backupId) {
    try {
      ...
      await optimizedSheets.deleteRow(tenantId, '_proofkit_backups', backupRecord);
