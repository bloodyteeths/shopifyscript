...
class IntentOSService {
  constructor() {
    ...
    this.templates = {
      ...
      services: {
        ...
        benefit_bullets: "Licensed Professionals|Satisfaction Guaranteed|Fast Response",
  ...
  async applyMetafieldOverlay(tenantId, overlayConfig, promote = false) {
    ...
    try {
      ...
      const result = await this.executeMetafieldOverlay(tenantId, overlayConfig, version);
      
      // Cache the active overlay
      tenantCache.set(tenantId, '/intent-os/overlay/active', {}, {
        version,
   ...
   * GATED BY PROMOTE FLAG
   */
  async revertMetafieldOverlay(tenantId, targetVersion = null, promote = false) {
    if (!promote && !this.checkPromoteFlag(tenantId)) {
      throw new Error('Intent OS mutations require PROMOTE flag to be enabled');
    }
    ...
    try {
      ...
      const result = await this.executeMetafieldOverlay(tenantId, target.config, target.version);
      
      // Record revert action
      await this.recordOverlayAction(tenantId, 'REVERT', { targetVersion: target.version }, Date.now());
  ...
  async generateUTMContent(tenantId, utmTerm, productContext = {}) {
    ...
    try {
      ...
      const strategy = this.utmContentMap[utmTerm] || this.utmContentMap['research'];
      ...
      tenantCache.set(tenantId, cacheKey, productContext, contentVariations, 3600000); // 1 hour
  ...
  async getIntentBlocks(tenantId, intentKey = null) {
    try {
      ...
      const sheet = await this.ensureSheet(doc, `INTENT_BLOCKS_${tenantId}`, [
        'intent_key', 'hero_headline', 'benefit_bullets_pipe', 'proof_snippet',
        'cta_text', 'url_target', 'updated_at', 'updated_by'
      ]);
      ...
      rows.forEach(row => {
        ...
        if (key) {
          blocks[key] = {
            ...
            benefit_bullets: (row.benefit_bullets_pipe || '').split('|').filter(Boolean),
  ...
  checkPromoteFlag(tenantId) {
    ...
    return tenant?.config?.INTENT_OS_PROMOTE === true || process.env.INTENT_OS_GLOBAL_PROMOTE === 'true';
  }
  ...
  async createOverlayVersion(tenantId, overlayConfig) {
    ...
    tenantCache.set(tenantId, versionKey, {}, {
      version,
      config: overlayConfig,
      created_at: new Date().toISOString()
    }, 2592000000); // 30 days
   ...
   */
  async generateContentVariations(tenantId, strategy, industry, productContext) {
...
Return as JSON with variations array.`;
    ...
    try {
      ...
      return aiResult?.variations || [this.templateToVariation(template, productContext)];
    } catch (error) {
  ...
  getFallbackContent(utmTerm) {
    return {
      ...
      variations: [{
        ...
        benefit_bullets: ["Quality Guaranteed", "Fast Shipping", "Expert Support"],
  ...
  async storePromoDraft(tenantId, draft) {
    try {
      ...
      const sheet = await this.ensureSheet(doc, `PROMO_DRAFTS_${tenantId}`, [
        'id', 'title', 'handle', 'status', 'content', 'meta_description',
        'created_at', 'created_by', 'tags_csv'
      ]);
