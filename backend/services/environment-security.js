...
class EnvironmentSecurity {
  ...
  validateEnvironmentConsistency() {
    ...
    if (this.startupEnv !== currentNodeEnv) {
      ...
      throw new Error('Environment manipulation detected - process terminating for security');
    }

    // Validate deployment environment
    if (this.deploymentEnv && !['development', 'staging', 'production'].includes(this.deploymentEnv)) {
      throw new Error(`Invalid DEPLOYMENT_ENVIRONMENT: ${this.deploymentEnv}`);
    }

    // Production deployment must have NODE_ENV=production
    if (this.deploymentEnv === 'production' && this.startupEnv !== 'production') {
      logger.error('SECURITY ALERT: Production deployment with non-production NODE_ENV', {
        deploymentEnv: this.deploymentEnv,
  ...
  isShopifyTestSafe() {
    ...
    const isTestDeployment = ['development', 'staging'].includes(this.deploymentEnv);
  ...
  checkShopifyTestAccountMarkers() {
    ...
    return testMarkers.some(marker => 
      shopifyUrl.includes(marker) || tenantId.includes(marker)
    );
  }
  ...
  validateSecurePromoteGate(tenant, mutationType = 'GENERAL') {
    ...
    if (this.isProductionExecution()) {
      logger.info('Secure PROMOTE Gate: Production deployment - full validation required', {
        tenant,
        mutationType,
    ...
    if (this.isShopifyTestSafe()) {
      logger.info('Secure PROMOTE Gate: Shopify test safe context - limited bypass allowed', {
        tenant,
        mutationType,
        environmentInfo: envInfo
      });
    ...
    logger.info('Secure PROMOTE Gate: Development deployment - bypass allowed with audit', {
      tenant,
      mutationType,
      environmentInfo: envInfo
    });
