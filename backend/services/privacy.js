...
class PrivacyService {
  ...
  async recordConsent(tenantId, userId, consentData) {
    try {
      ...
      const record = {
        ...
        timestamp: timestamp,
        expires_at:
          consentData.expiresAt || this.calculateConsentExpiry(timestamp),
      ...
      console.log(
        `Privacy: Consent recorded for tenant ${tenantId}, consent_id: ${consentId}`,
      );
  ...
  async withdrawConsent(tenantId, userId, consentId, reason = "user_request") {
    try {
      ...
      await this.triggerDataDeletion(
        tenantId,
        hashedUserId,
        "consent_withdrawn",
      );
      ...
      console.log(
        `Privacy: Consent withdrawn for tenant ${tenantId}, user: ${hashedUserId}`,
      );
  ...
  async processDataDeletionRequest(tenantId, userId, requestData = {}) {
    try {
      ...
      const deletionResult = await this.executeDeletion(
        tenantId,
        hashedUserId,
        deletionRequest,
      );
      ...
      deletionRequest.processing_status = deletionResult.success
        ? "completed"
        : "failed";
      deletionRequest.completed_at = new Date().toISOString();
      deletionRequest.deletion_summary = JSON.stringify(
      ...
      console.log(
        `Privacy: Data deletion processed for tenant ${tenantId}, deletion_id: ${deletionId}`,
      );
      return {
        success: true,
        deletionId,
  ...
  async executeDeletion(tenantId, hashedUserId, deletionRequest) {
    ...
    try {
      ...
      console.log(
        `Privacy: Deletion executed for ${tenantId}, deleted ${summary.records_deleted} records across ${summary.sheets_deleted} sheets`,
      );
  ...
  async exportUserData(tenantId, userId, format = "json") {
    try {
      ...
      if (doc) {
        ...
        for (const sheetType of dataSheets) {
          try {
            ...
            console.warn(
              `Privacy: Error exporting from ${sheetType}:`,
              error.message,
            );
          }
        }
      ...
      console.log(
        `Privacy: Data export completed for tenant ${tenantId}, export_id: ${exportId}`,
      );
      return {
        success: true,
        exportId,
        ...
        metadata: {
          ...
          record_count: Object.values(userDataExport.data).reduce(
  ...
  async getProcessingLog(tenantId, filters = {}) {
    try {
      ...
      if (filters.activity_type) {
        logs = logs.filter(
          (log) => log.activity_type === filters.activity_type,
        );
      }

      if (filters.from_date) {
        logs = logs.filter(
          (log) => new Date(log.timestamp) >= new Date(filters.from_date),
        );
      }

      if (filters.to_date) {
        logs = logs.filter(
          (log) => new Date(log.timestamp) <= new Date(filters.to_date),
        );
      }
  ...
  async checkRetentionCompliance(tenantId) {
    try {
      ...
      for (const [category, retentionDays] of Object.entries(
        this.retentionPeriods,
      )) {
      ...
      console.log(
        `Privacy: Retention compliance check completed for ${tenantId}, found ${complianceReport.violations.length} violations`,
      );
  ...
  async cleanupExpiredData(tenantId, dryRun = true) {
    try {
      ...
      console.log(
        `Privacy: Cleanup ${dryRun ? "simulation" : "execution"} completed for ${tenantId}, ${cleanupReport.cleaned_records} records processed`,
      );
    ...
    try {
      ...
      if (doc) {
        const sheet = await ensureSheet(
          doc,
          `CONSENT_RECORDS_${tenantId}`,
          this.getSheetHeaders("CONSENT_RECORDS"),
        );
  ...
  async storeConsentWithdrawal(tenantId, record) {
    try {
      ...
      if (doc) {
        const sheet = await ensureSheet(
          doc,
          `CONSENT_WITHDRAWALS_${tenantId}`,
          this.getSheetHeaders("CONSENT_WITHDRAWALS"),
        );
        await sheet.addRow(record);
      }
    } catch (error) {
  ...
  async logDataProcessing(tenantId, activity) {
    try {
      ...
      const record = {
        ...
        data_categories: Array.isArray(activity.data_categories)
      ...
      if (doc) {
        const sheet = await ensureSheet(
          doc,
          `DATA_PROCESSING_LOGS_${tenantId}`,
          this.getSheetHeaders("DATA_PROCESSING_LOGS"),
        );
        await sheet.addRow(record);
      }
    } catch (error) {
  ...
  async storeDeletionRequest(tenantId, record) {
    try {
      ...
      if (doc) {
        const sheet = await ensureSheet(
          doc,
          `DATA_DELETION_REQUESTS_${tenantId}`,
          this.getSheetHeaders("DATA_DELETION_REQUESTS"),
        );
        await sheet.addRow(record);
      }
    } catch (error) {
    ...
    try {
      ...
      if (doc) {
        ...
        const existingRow = rows.find(
          (row) => row.deletion_id === record.deletion_id,
        );
        if (existingRow) {
      ...
      if (doc) {
        ...
        for (const sheetType of financialSheets) {
          try {
            ...
            console.warn(
              `Privacy: Error pseudonymizing ${sheetType}:`,
              error.message,
            );
          }
        }
