...
app.get('/api/healthz', (req, res) => json(res, 200, { ok: true, ts: new Date().toISOString() }));
...
app.post('/api/overlays/apply', async (req, res) => {
  ...
  try {
    const { appendRows } = await getSheetOperations();
    try { 
      await appendRows(tenant, 'OVERLAY_HISTORY', ['timestamp','action','selector','channel','fields_json'], 
        [[new Date().toISOString(), 'apply', 
          (product_ids.length ? `products:${product_ids.length}` : 
           (collection_ids.length ? `collections:${collection_ids.length}` : 'none')), 
          channel, JSON.stringify(fields)]]); 
    } catch {}
    try { 
      await appendRows(tenant, 'RUN_LOGS', ['timestamp','message'], 
...
app.post('/api/overlays/bulk', async (req, res) => {
  ...
  try {
    ...
    try { 
      await appendRows(tenant, 'OVERLAY_HISTORY', ['timestamp','action','selector','channel','fields_json'], 
        [[new Date().toISOString(), 'apply_bulk', `${select}:${value}`, channel, JSON.stringify(fields)]]); 
    } catch {}
    try { 
      await appendRows(tenant, 'RUN_LOGS', ['timestamp','message'], 
...
app.post('/api/seed-demo', async (req, res) => {
  ...
  try {
    ...
    await metSheet.addRow({ 
      date: iso, level: 'ad_group', campaign: 'Demo Campaign', ad_group: 'Demo AdGroup', 
      id: '1', name: 'Demo KW', clicks: '12', cost: '6.50', conversions: '1', impr: '120', ctr: '0.10' 
    });
    ...
    await rlSheet.addRow({ 
      timestamp: new Date().toISOString(), message: 'seed_demo_data' 
    });
...
app.post('/api/promote/window', async (req, res) => {
  ...
  try {
    ...
    try {
      const { appendRows } = await getSheetOperations();
      await appendRows(tenant, 'RUN_LOGS', ['timestamp','message'], 
        [[new Date().toISOString(), `promote_window_scheduled:${duration_minutes}`]]); 
    } catch {}
...
app.get('/api/promote/status', async (req, res) => {
  ...
  try {
    const { getDoc, ensureSheet, readConfigFromSheets } = await getSheetOperations();
    const doc = await getDoc();
    const now = Date.now();
...
app.put('/api/alerts/channels/:tenant/:channelId', alertsConfigRoutes.updateAlertChannel);
app.delete('/api/alerts/channels/:tenant/:channelId', alertsConfigRoutes.deleteAlertChannel);
app.post('/api/alerts/channels/:tenant/:channelId/test', alertsConfigRoutes.testAlertChannel);

app.get('/api/alerts/anomaly-settings/:tenant', alertsConfigRoutes.getAnomalySettings);
app.put('/api/alerts/anomaly-settings/:tenant', alertsConfigRoutes.updateAnomalySettings);
app.post('/api/alerts/anomaly-detection/:tenant/run', alertsConfigRoutes.runAnomalyDetection);
app.post('/api/alerts/suppress/:tenant', alertsConfigRoutes.suppressAlert);

app.post('/api/alerts/weekly-summary/:tenant/generate', alertsConfigRoutes.generateWeeklySummary);
