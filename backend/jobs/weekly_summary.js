...
export async function runWeeklySummary(tenant, options = {}) {
  ...
  try {
    ...
    // Log to run logs
    const runLogs = await ensureSheet(doc, `RUN_LOGS_${tenant}`, [
      "timestamp",
      "type",
      "message",
      "data",
...
async function extractWeeklyData(doc, tenant) {
  ...
  const weeklyData = processWeekData(
    metricsRows,
    searchTermsRows,
    currentWeekStart,
    Date.now(),
  ...
  const previousWeekData = processWeekData(
    metricsRows,
    searchTermsRows,
    previousWeekStart,
    currentWeekStart,
...
function calculateInsights(current, previous) {
  ...
  insights.trends = {
    ...
    cpa:
      current.totals.cpa && previous.totals.cpa
...
function createInsightPrompt(current, previous, tenant) {
  const trends = {
    clicks:
      previous.totals.clicks > 0
        ? ((current.totals.clicks - previous.totals.clicks) /
            previous.totals.clicks) *
          100
        : 0,
    cost:
      previous.totals.cost > 0
        ? ((current.totals.cost - previous.totals.cost) /
            previous.totals.cost) *
          100
        : 0,
    conversions:
      previous.totals.conversions > 0
        ? ((current.totals.conversions - previous.totals.conversions) /
            previous.totals.conversions) *
          100
        : 0,
  };
...
TOP SEARCH TERMS: ${current.searchTerms
    .slice(0, 5)
    .map((t) => t.term)
    .join(", ")}
...
function createComprehensiveSummary(current, previous, insights, aiInsights) {
  ...
  if (insights.topPerformers.campaign) {
    ...
    plainTextLines.push(
      `• ${camp.name}: ${camp.conversions} conversions at ${formatCurrency(camp.cpa)} CPA`,
    );
    plainTextLines.push(``);
  }
  ...
  if (insights.topPerformers.searchTerms.length > 0) {
    ...
    insights.topPerformers.searchTerms.slice(0, 3).forEach((term) => {
      plainTextLines.push(
        `• "${term.term}": ${term.conversions} conversions, ${term.clicks} clicks`,
      );
    });
    plainTextLines.push(``);
  }
