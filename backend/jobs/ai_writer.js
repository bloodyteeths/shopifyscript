...
import { getApprovalWorkflow, CONTENT_TYPES } from '../services/content-approval.js';

function failFast(msg){ console.error(msg); process.exit(1); }

async function writeDrafts(tenant, themes){
  const doc = await getDoc(); if (!doc) return 0;
  const rsa = await ensureSheet(doc, `RSA_ASSETS_DEFAULT_${tenant}`, ['headlines_pipe','descriptions_pipe','theme','rationale','source_url','approval_status']);
  const lib = await ensureSheet(doc, `ASSET_LIBRARY_${tenant}`, ['theme','headline','description','tone','source','approval_status']);
  const sitelinks = await ensureSheet(doc, `SITELINKS_${tenant}`, ['link_text','line1','line2','final_url','theme','approval_status']);
  const callouts = await ensureSheet(doc, `CALLOUTS_${tenant}`, ['callout_text','theme','approval_status']);
  const snippets = await ensureSheet(doc, `SNIPPETS_${tenant}`, ['header','values_pipe','theme','approval_status']);
  ...
  for (const t of themes){
    try {
      ...
      if (rsaResult.success && rsaResult.content) {
        ...
        headlines = [ `${t} Deals`, `${t} Official Site`, `${t} Fast Shipping`, `${t} Start Free`, `${t} Trusted` ];
        descriptions = [ `${t} — shop now with fast shipping and easy returns.`, `${t} — compare options and find your best fit today.` ];
      }
      ...
      const approvalStatus = rsaApproval.success && rsaApproval.status === 'approved' ? 'approved' : 'pending';
...
async function main(){
  ...
  try { 
    ...
    const msg = 'AI disabled: set AI_PROVIDER=(openai|anthropic|google) and corresponding API key (see /docs/SECRETS.md).';
    console.log(msg);
    return process.exit(0);
  }

  const themes = Array.from({length: Math.max(1, Math.min(5, limit))}).map((_,i)=>`Theme ${i+1}`);
  if (dryRun){ 
