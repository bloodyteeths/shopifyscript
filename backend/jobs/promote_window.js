...
export async function schedulePromoteWindow(
  tenant,
  startAtMs,
  durationMinutes,
) {
  const doc = await getDoc();
  if (!doc) return { ok: false, error: "no_sheets" };
...
export async function tickPromoteWindow(tenant) {
  ...
  const row = rows[0];
  const now = Date.now();
  const start = Number(row.start_at_ms || 0),
    end = Number(row.end_at_ms || 0),
    state = String(row.state || "");
  const cfg = await ensureSheet(doc, `CONFIG_${tenant}`, ["key", "value"]);
  async function setPromote(val) {
    const all = await cfg.getRows();
    const map = {};
    all.forEach((r) => {
    ...
    await cfg.clearRows();
    await cfg.setHeaderRow(["key", "value"]);
    for (const k of Object.keys(map))
      await cfg.addRow({ key: k, value: map[k] });
  }
  if (now >= start && now < end && state !== "on") {
    await setPromote(true);
    row.state = "on";
    await row.save();
  } else if (now >= end && state !== "off_done") {
    await setPromote(false);
    row.state = "off_done";
    await row.save();
  }
}
