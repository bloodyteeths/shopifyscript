...
function evalSqlish(row, expr){
  ...
  function evalAtom(i){
    const field = tokens[i]||''; const op = tokens[i+1]||''; let value = tokens[i+2]||'';
  ...
  let i=0; let acc = evalAtom(0); i+=3;
  while(i<tokens.length){
    const join = (tokens[i]||'').toUpperCase(); i+=1; const part = evalAtom(i); i+=3;
    if (join === 'AND') acc = acc && part; else if (join === 'OR') acc = acc || part; else return acc;
  }
  return acc;
}

async function readSeeds(doc, tenant){
  const sh = await ensureSheet(doc, `AUDIENCE_SEEDS_${tenant}`, ['customer_id','email','phone','first_name','last_name','country','zip','consent_status','last_order_at']);
  const rows = await sh.getRows();
  return rows.map((r)=>({
...
export async function buildSegments(tenant, segments, format){
  const doc = await getDoc(); if (!doc) return { built: [], skipped: segments };
  ...
  for (const segKey of segments){
    ...
    const seg = segs.find(s=>s.segment_key===segKey); if (!seg){ out.push({ segment:segKey, rows:0, url:'' }); continue; }
    // filter seeds
    const look = Number(seg.lookback_days||0);
    const lookTs = look>0 ? Date.now() - look*24*3600*1000 : 0;
    const filtered = seeds.filter(r => (r.consent_status||'granted').toLowerCase()==='granted')
      .filter(r => {
        if (!lookTs || !r.last_order_at) return true;
        const t = Date.parse(r.last_order_at); return isFinite(t) ? (t>=lookTs) : true;
      })
      .filter(r => evalSqlish(r, seg.logic_sqlish));
    const ts = new Date().toISOString().replace(/[:.]/g,'-');
    const file = path.resolve(dir, `${seg.segment_key}-${ts}.${format==='API'?'API':'UI'}.csv`);
    let header = '';
    let lines = [];
    if (format==='API'){
      ...
      lines = filtered.map(r=>[
        ...
        r.first_name||'', r.last_name||'', r.country||'', r.zip||''
      ].join(','));
    }
    fs.writeFileSync(file, header + '\n' + lines.join('\n'), 'utf8');
    const rel = `storage/audiences/${tenant}/${path.basename(file)}`;
    const shExp = await ensureSheet(doc, `AUDIENCE_EXPORT_${tenant}`, ['file_name','segment_key','format','row_count','last_built_at','storage_url']);
    await shExp.addRow({ file_name: path.basename(file), segment_key: seg.segment_key, format: (format==='API'?'CM_API_HASHED':'CM_UI_UNHASHED'), row_count: lines.length, last_built_at: new Date().toISOString(), storage_url: rel });
