...
class SecurityMiddleware {
  constructor() {
    ...
    this.ddosProtection = {
      ...
      thresholds: {
        ...
        concurrent_connections: Number(process.env.DDOS_CONCURRENT_THRESHOLD || 100),
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        if (this.isIPBanned(clientIP)) {
          return this.blockRequest(res, 'IP_BANNED', 'IP address is temporarily banned');
        }
        ...
        if (!rateLimitCheck.allowed) {
          return this.blockRequest(res, 'RATE_LIMITED', rateLimitCheck.reason, rateLimitCheck.retryAfter);
        }
  ...
  async checkDDoSProtection(req, clientIP) {
    ...
    if (contentLength > this.ddosProtection.thresholds.payload_size_mb * 1024 * 1024) {
    ...
    const concurrentConnections = this.ddosProtection.connectionCount.get(clientIP) || 0;
    if (concurrentConnections > this.ddosProtection.thresholds.concurrent_connections) {
    ...
    if (now - requestCounts.lastReset > 60000) { // 1 minute
      requestCounts.minute = 0;
      requestCounts.second = 0;
      requestCounts.lastReset = now;
    } else if (now - requestCounts.lastReset > 1000) { // 1 second
      requestCounts.second = 0;
    }
    ...
    // Check thresholds
    if (requestCounts.second > this.ddosProtection.thresholds.requests_per_second) {
      await this.addStrike(clientIP, 'HIGH_FREQUENCY_REQUESTS');
      return { allowed: false, reason: 'Too many requests per second' };
    }
    
    if (requestCounts.minute > this.ddosProtection.thresholds.requests_per_minute) {
  ...
  validateAndSanitizeInput(req) {
    ...
    try {
      ...
      return {
        valid: false,
        violations: [{ type: 'VALIDATION_ERROR', field: 'unknown', value: 'error' }],
  ...
  async detectThreats(req, clientIP) {
    ...
    return {
      ...
      threats: threats,
      severity: severity,
      reason: threats.length > 0 ? `Threats detected: ${threats.map(t => t.type).join(', ')}` : null
    };
  }
  ...
  detectSuspiciousPatterns(clientIP, req) {
    ...
    if (patterns.userAgents.size > 5) {
      return { detected: true, type: 'MULTIPLE_USER_AGENTS', severity: 'MEDIUM', reason: 'Multiple user agents from same IP' };
    }
    
    if (patterns.endpoints.size > 20) {
      return { detected: true, type: 'ENDPOINT_SCANNING', severity: 'HIGH', reason: 'Scanning multiple endpoints' };
    }
    
    if (patterns.timings.length > 100) {
      return { detected: true, type: 'HIGH_FREQUENCY_PATTERN', severity: 'HIGH', reason: 'Abnormally high request frequency' };
    }
  ...
  checkKnownAttackPatterns(req) {
    ...
    const attackPatterns = [
      { pattern: /\.\.\/|\.\.\\|\.\.%2f|\.\.%5c/i, type: 'DIRECTORY_TRAVERSAL', severity: 'HIGH' },
      { pattern: /\/proc\/|\/etc\/passwd|\/etc\/shadow/i, type: 'FILE_ACCESS_ATTEMPT', severity: 'HIGH' },
      { pattern: /phpinfo|eval\(|base64_decode|exec\(/i, type: 'CODE_INJECTION', severity: 'HIGH' },
      { pattern: /\bwget\b|\bcurl\b|\bscp\b|\bsftp\b/i, type: 'REMOTE_ACCESS_ATTEMPT', severity: 'HIGH' },
      { pattern: /<script|javascript:|vbscript:|onload=|onerror=/i, type: 'XSS_ATTEMPT', severity: 'MEDIUM' },
      { pattern: /union.*select|insert.*into|delete.*from|drop.*table/i, type: 'SQL_INJECTION', severity: 'HIGH' },
      { pattern: /\${|<%|%{|#\{/i, type: 'TEMPLATE_INJECTION', severity: 'MEDIUM' },
      { pattern: /\.php|\.asp|\.jsp|\.cgi/i, type: 'FILE_EXTENSION_PROBE', severity: 'LOW' }
    ];
  ...
  detectBehavioralAnomalies(clientIP, req) {
    ...
    baseline.methodDistribution.set(req.method, (baseline.methodDistribution.get(req.method) || 0) + 1);
    ...
    if (baseline.requestCount > 20) {
      const avgSize = baseline.requestSizes.reduce((a, b) => a + b, 0) / baseline.requestSizes.length;
      const variance = baseline.requestSizes.reduce((sum, size) => sum + Math.pow(size - avgSize, 2), 0) / baseline.requestSizes.length;
      const stdDev = Math.sqrt(variance);
      
      if (stdDev > 0 && Math.abs(requestSize - avgSize) > this.threatDetection.anomalyThresholds.requestSizeVariance * stdDev) {
        threats.push({
          type: 'REQUEST_SIZE_ANOMALY',
          severity: 'MEDIUM',
          details: { currentSize: requestSize, avgSize, deviation: Math.abs(requestSize - avgSize) / stdDev }
        });
      }
      ...
      if (baseline.endpointPatterns.size > 50) {
        threats.push({
          ...
          severity: 'HIGH',
          details: { uniqueEndpoints: baseline.endpointPatterns.size, requests: baseline.requestCount }
        });
      }
    }
  ...
  detectBotActivity(req, userAgent) {
    ...
    if (!req.headers.accept || !req.headers['accept-language']) {
      threats.push({
        ...
        severity: 'MEDIUM',
        details: { missingHeaders: ['accept', 'accept-language'].filter(h => !req.headers[h]) }
      });
    }
  ...
  detectReconnaissance(clientIP, req) {
    ...
    if (path.includes('404') || path.includes('500') || path.includes('error')) {
      threats.push({
  ...
  detectBruteForce(clientIP, req) {
    ...
    if (isAuthEndpoint && req.method === 'POST') {
      ...
      if (attempts.count > 10 && timeWindow < 5 * 60 * 1000) { // 10 attempts in 5 minutes
        threats.push({
  ...
  getDynamicLimits(tenantId, endpoint) {
    ...
    const adjustments = this.rateLimiting.dynamicLimits.get(dynamicKey) || { multiplier: 1.0 };
  ...
  adjustDynamicLimits(tenantId, endpoint, bucket) {
    const dynamicKey = `${tenantId}:${endpoint}`;
    const current = this.rateLimiting.dynamicLimits.get(dynamicKey) || { multiplier: 1.0, adjustedAt: Date.now() };
    
    // Adjust based on usage patterns
    const usageRatio = bucket.count / (bucket.resetTime - bucket.firstRequest) * 1000; // requests per second
  ...
  async logSecurityEvent(clientIP, eventType, details) {
    ...
    try {
      ...
      if (doc) {
        const sheet = await ensureSheet(doc, 'SECURITY_EVENTS', [
          'timestamp', 'client_ip', 'event_type', 'details', 'severity'
        ]);
    ...
    console.log(`Security: Event logged - ${eventType} for IP ${clientIP.substring(0, 8)}***`);
  }

  hashIP(ip) {
    return crypto.createHash('sha256').update(String(ip)).digest('hex').substring(0, 12);
  }
  ...
  cleanupExpiredEntries() {
    ...
    for (const [ip, baseline] of this.threatDetection.behaviorBaselines) {
      if (now - baseline.firstSeen > 24 * 60 * 60 * 1000) { // 24 hours
  ...
  processAlertQueue() {
    ...
    alerts.forEach(alert => {
      console.warn(`Security Alert: ${alert.event_type} from IP ${alert.client_ip}`);
    });
  }
  ...
  getSecurityStats() {
    return {
      ddos_protection: {
        ...
        active_connections: Array.from(this.ddosProtection.connectionCount.values()).reduce((a, b) => a + b, 0)
      },
      rate_limiting: {
