...
class TenantRateLimiter {
  ...
  async getTenantLimits(tenantId, endpoint) {
    ...
    try {
      ...
      console.warn(
        `RateLimiter: Failed to get tenant limits for ${tenantId}:`,
        error.message,
      );
  ...
  cleanup() {
    ...
    for (const [key, bucket] of this.buckets) {
      if (now - bucket.lastReset > 300000) {
        // 5 minutes
  ...
  getStats() {
    const now = Date.now();
    const tenantBuckets = Array.from(this.buckets.entries()).map(
      ([key, bucket]) => {
    ...
    const globalBuckets = Array.from(this.globalBuckets.entries()).map(
      ([key, bucket]) => {
