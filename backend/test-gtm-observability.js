...
async function setupTestData() {
  ...
  try {
    ...
    const metricsSheet = await ensureSheet(doc, `METRICS_${testConfig.tenant}`, [
      'date', 'level', 'campaign', 'ad_group', 'id', 'name', 'clicks', 'cost', 'conversions', 'impr', 'ctr'
    ]);

    const searchTermsSheet = await ensureSheet(doc, `SEARCH_TERMS_${testConfig.tenant}`, [
    ...
    for (let i = 30; i >= 0; i--) {
      ...
      sampleData.push({
        ...
        conversions: Math.floor(baseConversions * conversionMultiplier).toString(),
      ...
      searchTermsData.push({
        ...
        conversions: Math.floor(baseConversions * 0.4 * conversionMultiplier).toString()
      });
      ...
      searchTermsData.push({
        ...
        conversions: Math.floor(baseConversions * 0.3 * conversionMultiplier).toString()
      });
    }
    ...
    console.log(`✅ Test data created: ${sampleData.length} metrics rows, ${searchTermsData.length} search terms rows`);
    return true;
...
async function testWeeklySummary() {
  const result = await runWeeklySummary(testConfig.tenant, { generateAI: false });
  ...
  assert(typeof result.duration === 'number', 'Weekly summary should report duration');
  
  // Test with AI insights
  const resultWithAI = await runWeeklySummary(testConfig.tenant, { generateAI: true });
  assert(resultWithAI.ok, 'Weekly summary with AI should succeed');
  
  if (testConfig.verbose) {
    console.log('Sample weekly summary:', JSON.stringify(result.summary, null, 2));
  }
}
...
async function testAnomalyDetection() {
  ...
  const results = await anomalyDetectionService.detectAnomalies(testConfig.tenant, '30d');
  
  assert(typeof results === 'object', 'Anomaly detection should return results object');
  assert(Array.isArray(results.alerts), 'Results should contain alerts array');
  assert(Array.isArray(results.warnings), 'Results should contain warnings array');
  assert(results.timestamp, 'Results should contain timestamp');
  
  // We expect to find the cost spike we injected
  assert(results.alerts.length > 0 || results.warnings.length > 0, 'Should detect the injected anomaly');
  
  const summary = anomalyDetectionService.getAnomalySummary(testConfig.tenant);
  assert(summary.tenant === testConfig.tenant, 'Anomaly summary should be for correct tenant');
  assert(typeof summary.totalRuns === 'number', 'Summary should include run count');
...
async function testAlertChannels() {
  ...
  if (testConfig.verbose) {
    console.log('Configured channels:', channels.map(c => ({ type: c.type, name: c.name })));
  }
}
...
async function testAlertSending() {
  ...
  try {
    ...
    assert(typeof result === 'object', 'Send alert should return result object');
    assert(typeof result.success === 'boolean', 'Result should include success status');
  ...
  assert(typeof stats.totalDeliveries === 'number', 'Stats should include delivery count');
}
...
async function testJobScheduler() {
  ...
  assert(status.tenants.includes(testConfig.tenant), 'Should include test tenant');
  assert(typeof status.jobs === 'object', 'Status should include jobs');

  // Test manual job execution
  try {
    const anomalyResult = await jobScheduler.triggerJob('anomaly_detection', testConfig.tenant);
    assert(typeof anomalyResult === 'object', 'Manual anomaly detection job should return result');
    
    const summaryResult = await jobScheduler.triggerJob('weekly_summary', testConfig.tenant);
    assert(typeof summaryResult === 'object', 'Manual weekly summary job should return result');
  ...
  assert(!updatedStatus.jobs.health_check.enabled, 'Should disable health check job');
...
async function testLookerTemplate() {
  try {
    ...
    const templatePath = path.resolve(process.cwd(), 'looker-studio', 'proofkit-template.json');
    ...
    assert(primaryConnection.type === 'google_sheets', 'Primary connection should be Google Sheets');
    ...
    assert(sheetNames.includes('METRICS_{TENANT}'), 'Should include metrics sheet');
    assert(sheetNames.includes('SEARCH_TERMS_{TENANT}'), 'Should include search terms sheet');
    assert(sheetNames.includes('RUN_LOGS_{TENANT}'), 'Should include run logs sheet');
    assert(sheetNames.includes('ANOMALIES_{TENANT}'), 'Should include anomalies sheet');
  ...
  try {
    ...
    if (testConfig.runAll) {
      ...
      await runTest('End-to-End Workflow', async () => {
        ...
        const anomalyResult = await anomalyDetectionService.detectAnomalies(testConfig.tenant);
        assert(anomalyResult.alerts || anomalyResult.warnings, 'E2E: Should detect anomalies');
        
        // Send alerts for anomalies
        for (const alert of anomalyResult.alerts.slice(0, 1)) { // Test just one
          try {
