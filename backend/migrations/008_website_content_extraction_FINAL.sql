-- =====================================================
-- Migration 008: Website Content Extraction System (FINAL VERSION)
-- Created: 2025-09-28
-- Description: Complete fix for all missing columns in all tables
-- =====================================================

-- STEP 1: Fix website_content table completely
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'website_content') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'website_content' AND column_name = 'tenant_id') THEN
            ALTER TABLE website_content ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'website_content' AND column_name = 'url') THEN
            ALTER TABLE website_content ADD COLUMN url TEXT NOT NULL DEFAULT '';
        END IF;
...
-- STEP 2: Fix content_index table completely - INCLUDING content_text!
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_index') THEN
        -- Add ALL missing columns including content_text
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_index' AND column_name = 'tenant_id') THEN
            ALTER TABLE content_index ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_index' AND column_name = 'website_content_id') THEN
            ALTER TABLE content_index ADD COLUMN website_content_id UUID;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_index' AND column_name = 'content_type') THEN
            ALTER TABLE content_index ADD COLUMN content_type TEXT NOT NULL DEFAULT 'general';
        END IF;
        -- THIS IS THE MISSING COLUMN!
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_index' AND column_name = 'content_text') THEN
            ALTER TABLE content_index ADD COLUMN content_text TEXT NOT NULL DEFAULT '';
        END IF;
...
-- STEP 3: Fix content_tags table
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_tags') THEN
        -- Add missing columns
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_tags' AND column_name = 'content_index_id') THEN
            ALTER TABLE content_tags ADD COLUMN content_index_id UUID;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_tags' AND column_name = 'tag_name') THEN
            ALTER TABLE content_tags ADD COLUMN tag_name TEXT NOT NULL DEFAULT '';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_tags' AND column_name = 'tag_category') THEN
            ALTER TABLE content_tags ADD COLUMN tag_category TEXT;
        END IF;
...
-- STEP 4: Create content_extraction_log if missing
...
-- STEP 6: Create all indexes (safe with IF NOT EXISTS)
...
-- Now that content_text column exists, create the full-text search index
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_index' AND column_name = 'content_text') THEN
...
-- STEP 7: Drop and recreate views safely
...
-- STEP 9: Create/recreate policies
...
-- STEP 10: Create/recreate functions and triggers
...
-- Create content quality function
...
-- SUCCESS!
SELECT 'Migration 008: Website Content Extraction (FINAL) - COMPLETE!' AS status,
       'All tables, columns, and indexes have been created successfully!' AS message;