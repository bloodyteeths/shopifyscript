-- =====================================================
-- Migration 010: Dashboard Views (ULTIMATE)
-- Created: 2025-09-28
-- Description: Creates all missing tables, columns, and views for AI Dashboard
-- =====================================================

-- STEP 1: Create missing traffic_patterns table (not in 009 migration)
...
-- Create indexes for traffic_patterns
CREATE INDEX IF NOT EXISTS idx_traffic_patterns_tenant ON traffic_patterns(tenant_id);
CREATE INDEX IF NOT EXISTS idx_traffic_patterns_date ON traffic_patterns(date DESC);
CREATE INDEX IF NOT EXISTS idx_traffic_patterns_tenant_date ON traffic_patterns(tenant_id, date DESC);

-- STEP 2: Ensure ALL required columns exist in ALL tables
DO $$
BEGIN
    -- Add ALL missing columns to content_extraction_log
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_extraction_log') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                       WHERE table_name = 'content_extraction_log'
                       AND column_name = 'started_at') THEN
            ALTER TABLE content_extraction_log ADD COLUMN started_at TIMESTAMP DEFAULT NOW();
        END IF;
    ...
    -- Add missing columns to website_content
    ...
    -- Add missing columns to traffic_predictions
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'traffic_predictions') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    ...
    -- Add missing columns to traffic_anomalies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'traffic_anomalies') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
            ...
            ALTER TABLE traffic_anomalies ADD COLUMN created_at TIMESTAMP DEFAULT NOW();
        END IF;
    END IF;

    -- Add missing columns to ad_schedule_configs
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ad_schedule_configs') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- STEP 3: Drop existing views to recreate them
...
SELECT
    ...
    MAX(COALESCE(wc.scraped_at, wc.last_scraped)) as last_content_scrape,
    ...
    CASE
        WHEN MAX(COALESCE(wc.scraped_at, wc.last_scraped)) < NOW() - INTERVAL '7 days' THEN 'stale'
        WHEN MAX(COALESCE(wc.scraped_at, wc.last_scraped)) < NOW() - INTERVAL '3 days' THEN 'aging'
        ELSE 'fresh'
...
-- 3. Content Insights View (Simplified to avoid column issues)
...
GROUP BY wc.id, wc.tenant_id, wc.url, wc.domain, wc.content_type,
         ...
         wc.testimonials, wc.offers;

-- 4. Traffic Overview View
CREATE VIEW dashboard_traffic_overview AS
SELECT
    tp.tenant_id,
    tp.prediction_date,
    tp.predicted_clicks,
    COALESCE(tp.predicted_impressions, 0) as predicted_impressions,
    COALESCE(tp.predicted_ctr, 0) as predicted_ctr,
    tp.predicted_conversions,
    tp.confidence_score,
    tp.efficiency_score,

    -- Historical Comparison (from traffic_patterns table)
    ht.avg_clicks_7d,
    ...
    CASE
        WHEN tp.predicted_clicks > COALESCE(ht.avg_clicks_7d, 0) * 1.2 THEN 'growth_expected'
        WHEN tp.predicted_clicks < COALESCE(ht.avg_clicks_7d, 0) * 0.8 THEN 'decline_expected'
        ELSE 'stable'
    END as trend,
...
-- 6. Recent Activity View (Simplified to avoid issues)
...
-- Handle both possible column names for website_content
DO $$
BEGIN
...
GRANT ALL ON traffic_patterns TO service_role;

-- Success message
SELECT 'Migration 010: Dashboard Views (ULTIMATE) - SUCCESS!' AS status,
       'Created traffic_patterns table and all dashboard views' AS message,
       'All missing columns added including started_at' AS details;