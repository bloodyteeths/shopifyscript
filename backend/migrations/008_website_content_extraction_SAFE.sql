-- =====================================================
-- Migration 008: Website Content Extraction System (SAFE VERSION)
-- Created: 2025-09-28
-- Description: Safe migration that checks existing schema
-- =====================================================

-- First, let's check what exists and add missing columns if needed
DO $$
BEGIN
    -- Check if website_content table exists
...
-- Create indexes (IF NOT EXISTS handles existing ones)
...
-- Create indexes for content_index
...
-- Create index for extraction log
...
-- Drop and recreate views to ensure they use the correct schema
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables
        WHERE tablename = 'content_extraction_log'
        AND rowsecurity = true
    ) THEN
...
-- Migration complete
SELECT 'Migration 008: Website Content Extraction (SAFE) - COMPLETE' AS status;