-- =====================================================
-- Migration 010: Dashboard Views (FIXED)
-- Created: 2025-09-28
-- Description: Optimized views for AI Dashboard
-- Fixed: Changed 'asc' alias to 'asc_config' to avoid reserved keyword conflict
-- =====================================================
...
-- 3. Content Insights View - Website content analysis
...
LEFT JOIN (
    SELECT
        ...
        MAX(EXTRACT(EPOCH FROM (completed_at - started_at))/60) as extraction_duration_minutes,
...
GROUP BY wc.id, wc.tenant_id, wc.url, wc.domain, wc.content_type,
         wc.scraped_at, wc.quality_score, wc.products, wc.usps,
...
-- 6. Recent Activity View - Latest system events
...
-- Success message
SELECT 'Migration 010: Dashboard Views (FIXED) - COMPLETE' AS status;