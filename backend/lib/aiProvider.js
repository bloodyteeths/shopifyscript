...
async function createOpenAIProvider() {
  ...
  return { provider: 'openai', generateText, remainingCalls: () => maxCalls - calls };
}
...
async function createAnthropicProvider() {
  ...
  if (!key) throw new AIProviderError('Anthropic API key required', 'anthropic');
  ...
  return { provider: 'anthropic', generateText, remainingCalls: () => maxCalls - calls };
}
...
async function createGoogleProvider() {
  ...
  return { provider: 'google', generateText, remainingCalls: () => maxCalls - calls };
}
...
export async function getAIProvider() {
  ...
  const fallbackProviders = (process.env.AI_FALLBACK_PROVIDERS || '').toLowerCase().split(',').filter(Boolean);
  ...
  throw new AIProviderError('No AI provider available. Set AI_PROVIDER to openai, anthropic, or google with corresponding API key.');
}
...
export function getAIProviderSync() {
  ...
  if (provider === 'google') {
    const key = process.env.GOOGLE_API_KEY || '';
    if (!key) throw new Error('AI disabled: set AI_PROVIDER=google and GOOGLE_API_KEY (see /docs/SECRETS.md).');
  ...
  throw new Error('AI disabled: set AI_PROVIDER=google and GOOGLE_API_KEY (see /docs/SECRETS.md).');
}
