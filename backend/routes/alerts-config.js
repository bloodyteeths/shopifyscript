...
export async function getAlertChannels(req, res) {
  try {
    ...
    logger.error("Failed to get alert channels", {
      error: error.message,
...
export async function getAnomalySettings(req, res) {
  try {
    ...
    logger.error("Failed to get anomaly settings", {
      error: error.message,
      tenant: req.params.tenant,
    });
    res.status(500).json({
...
export async function updateAnomalySettings(req, res) {
  try {
    ...
    logger.error("Failed to update anomaly settings", {
      error: error.message,
      tenant: req.params.tenant,
    });
    res.status(500).json({
...
export async function runAnomalyDetection(req, res) {
  try {
    ...
    logger.error("Failed to run anomaly detection", {
      error: error.message,
      tenant: req.params.tenant,
    });
    res.status(500).json({
...
export async function suppressAlert(req, res) {
  try {
    ...
    anomalyDetectionService.suppressAlert(
      tenant,
      alertType,
      severity,
      durationMs,
  ...
  switch (type) {
    ...
    case "email":
      ...
      if (config.recipients) {
        const invalidEmails = config.recipients.filter(
  ...
  for (const field of numericFields) {
    if (thresholds[field] !== undefined) {
      ...
      if (field === "zero_conversions_hours" && value > 168) {
        errors.push(
          "zero_conversions_hours should not exceed 168 hours (1 week)",
        );
      }
    }
  }
