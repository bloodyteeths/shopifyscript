...
router.get("/summary", async (req, res) => {
  ...
  try {
    ...
    const metricRows = await sheets.getRows(String(tenant), "METRICS", {
      limit: 1000,
    });
    let spend = 0,
      clicks = 0,
    ...
    const termRows = await sheets.getRows(String(tenant), "SEARCH_TERMS", {
      limit: 1000,
    });
...
router.post("/seed-demo", async (req, res) => {
  ...
  try {
    ...
    const stSheet = await ensureSheet(
      doc,
