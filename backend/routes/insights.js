...
function parseTsLoose(row) {
  ...
  if (Number.isFinite(n)) {
    ...
    if (n > 10_000 && n < 1_000_000) { // Google Sheets serial days
...
router.get('/insights', async (req, res) => {
  ...
  try {
    ...
    const metAoA = await sheets.getRows(String(tenant), 'METRICS', { limit: 4000 });
    const stsAoA = await sheets.getRows(String(tenant), 'SEARCH_TERMS', { limit: 4000 });
    ...
    for (const r of metObj) {
      ...
      const k = roundKey(ts);
      const cur = seriesMap.get(k) || { t: k, clicks: 0, cost: 0, conv: 0, impr: 0 };
      cur.clicks += Number(r.clicks || 0);
      cur.cost += Number(r.cost || 0);
...
router.get('/insights/terms', async (req, res) => {
  ...
  try {
    const ST_HEADERS = ['date','campaign','ad_group','search_term','clicks','cost','conversions'];
    const rowsAoA = await sheets.getRows(String(tenant), 'SEARCH_TERMS', { limit: 5000 });
    const toObj = (r) => ({
      date: r[0], campaign: r[1], ad_group: r[2], search_term: r[3],
      clicks: Number(r[4] || 0), cost: Number(r[5] || 0), conversions: Number(r[6] || 0)
    });
      ...
      if (doc) {
        ...
        try {
          const mapSheet = await ensureSheet(doc, `NEGATIVE_MAP_${tenant}`, 
            ['scope','campaign','ad_group','match','term']);
    ...
    rows = rows.map(r => {
      ...
      return {
        ...
        is_negative: negSet.has(termLc) || negAccPhrase.has(termLc) || 
                    negCampaigns.has(termLc) || negAdGroups.has(termLc),
...
router.get('/insights/terms.csv', async (req, res) => {
  ...
  try {
    ...
    const w = String(req.query.w || '7d').toLowerCase();
    const limit = Math.min(2000, Math.max(10, parseInt(String(req.query.page_size || '1000'), 10)));
    ...
    res.setHeader('Content-Disposition', `attachment; filename="terms_${tenant}_${w}.csv"`);
    return res.send(csv);
  } catch (e) {
...
router.get('/insights/debug', async (req, res) => {
  ...
  try {
    const MET_HEADERS = ['date','level','campaign','ad_group','id','name','clicks','cost','conversions','impr','ctr'];
    const ST_HEADERS = ['date','campaign','ad_group','search_term','clicks','cost','conversions'];
    const metAoA = await sheets.getRows(String(tenant), 'METRICS', { limit: 50 });
    const stsAoA = await sheets.getRows(String(tenant), 'SEARCH_TERMS', { limit: 50 });
    ...
    const preview = (rows) => rows.slice(-5).map(r => {
      ...
      if (raw instanceof Date) parsed = raw.getTime();
        ...
        if (Number.isFinite(n)) {
          ...
          else if (n > 10_000 && n < 1_000_000) parsed = (n - 25569) * 86400 * 1000;
        }
