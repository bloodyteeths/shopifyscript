...
import ShopifyBillingService, {
  SHOPIFY_PRICING_TIERS,
} from "../services/shopify-billing.js";
...
router.get("/pricing", (req, res) => {
  try {
    ...
    const tiers =
      platform === "shopify" ? SHOPIFY_PRICING_TIERS : PRICING_TIERS;
...
router.get("/stripe/subscriptions/:customerId", async (req, res) => {
  try {
    ...
    const subscriptions =
      await billingService.getCustomerSubscriptions(customerId);
 ...
 */
router.post(
  "/stripe/subscriptions/:subscriptionId/change",
  async (req, res) => {
    try {
      ...
      // Update subscription
      const updatedSubscription = await billingService.changeSubscription(
        subscriptionId,
        newTierIndex,
      );
...
router.get("/shopify/subscription/:shop", async (req, res) => {
  try {
    ...
    const shopifyBilling = new ShopifyBillingService();
    const subscription = await shopifyBilling.getCurrentSubscription(
      shop,
      accessToken,
    );
 ...
 */
router.post(
  "/shopify/subscription/:subscriptionId/change",
  async (req, res) => {
    try {
      const { subscriptionId } = req.params;
 ...
 */
router.post(
  "/shopify/subscription/:subscriptionId/cancel",
  async (req, res) => {
    try {
      const { subscriptionId } = req.params;
      ...
      const shopifyBilling = new ShopifyBillingService();
      const canceledSubscription = await shopifyBilling.cancelSubscription(
        shop,
        accessToken,
        subscriptionId,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await webhookHandler.processStripeWebhook(
        event,
        sig,
        rawBody,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await webhookHandler.processShopifyWebhook(
        topic,
        shop,
        data,
        signature,
...
router.get(
  "/ai-copywriter",
  requireFeature("ai_copywriter", { requiredTier: "Pro" }),
  (req, res) => {
    res.json({
      success: true,
...
function getAllFeatures() {
  return [
    ...
    },
    {
      ...
      description:
        "Daily optimization with budget caps, CPC ceilings, and scheduling",
    ...
    },
    {
      ...
      name: "AI Copywriter",
      description:
        "AI-generated responsive search ads with 30/90 character limits",
      category: "Creative",
    },
    {
      id: "rsa_test_queue",
      name: "RSA Test Queue",
      description:
        "Systematic testing of new ad variations with statistical significance",
      category: "Testing",
    },
    {
