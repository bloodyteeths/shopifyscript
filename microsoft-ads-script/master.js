...
function main() {
  ...
  var stRows = autoNegateAndCollectST_(
    cfg,
    cfg.st_lookback,
    cfg.st_min_clicks,
    cfg.st_min_cost,
...
function postToBackend_(action, payload) {
  var sig = sign_(
    "POST:" + TENANT_ID + ":" + action + ":" + (payload.nonce || ""),
  );
  var url =
    BACKEND_URL +
    "/" +
...
function getCampaigns_() {
  ...
  while (campaignIterator.hasNext()) {
    ...
    if (
      campaign.getType() === "SEARCH_AND_CONTENT" ||
      campaign.getType() === "SEARCH"
    ) {
...
function applyBudgetCaps_(campaigns, cfg) {
  campaigns.forEach(function (campaign) {
    ...
    var cap =
      cfg.BUDGET_CAPS[campaign.getName()] != null
        ? cfg.BUDGET_CAPS[campaign.getName()]
        : cfg.daily_budget_cap_default;
...
function applyBiddingStrategies_(campaigns, cfg) {
  campaigns.forEach(function (campaign) {
    ...
    var ceiling =
      cfg.CPC_CEILINGS[campaign.getName()] != null
        ? cfg.CPC_CEILINGS[campaign.getName()]
        : cfg.cpc_ceiling_default;
...
function attachNegativeList_(campaign, list) {
  ...
  if (!PREVIEW_MODE && NEG_GUARD_ACTIVE) {
    ...
    log_(
      "• Master negative list attachment planned for " +
        campaign.getName() +
        (PREVIEW_MODE ? " [PREVIEW]" : " [NEG_GUARD]"),
    );
  }
...
function autoNegateAndCollectST_(cfg, lookback, minClicks, minCost) {
  ...
  var query =
    "SELECT CampaignName, AdGroupId, AdGroupName, SearchQuery, Clicks, Spend, Conversions " +
...
function buildResponsiveAds_(cfg) {
  ...
  while (adGroupIterator.hasNext()) {
    ...
    var override =
      (cfg.RSA_MAP[campaignName] && cfg.RSA_MAP[campaignName][adGroupName]) ||
      null;
    var headlines =
      override && override.H && override.H.length
        ? override.H
        : cfg.RSA_DEFAULT.H || getDefaultHeadlines_();
    var descriptions =
      override && override.D && override.D.length
        ? override.D
        : cfg.RSA_DEFAULT.D || getDefaultDescriptions_();
    ...
    for (var campaignName in audienceMap) {
      ...
      for (var adGroupName in adGroupMap) {
        ...
        if (!audienceRow || !audienceRow.user_list_id) {
          log_(
            "! Missing user_list_id for " + campaignName + " › " + adGroupName,
          );
          errors++;
          continue;
        }
        ...
        var bidModifier = audienceRow.bid_modifier
          ? Number(audienceRow.bid_modifier)
          : null;
        ...
        try {
          ...
          if (!PREVIEW_MODE && cfg.PROMOTE) {
            ...
            if (operation && operation.isSuccessful()) {
              ...
              log_(
                "! Failed to attach audience " + listId + " to " + campaignName,
              );
              errors++;
            }
          } else {
            log_(
              "• Audience attachment planned: " +
                campaignName +
                " id=" +
                listId +
                " mode=" +
...
function collectPerformance_() {
  ...
  var campaignQuery =
    "SELECT CampaignId, CampaignName, Clicks, Spend, Conversions, Impressions, Ctr " +
  ...
  var adGroupQuery =
    "SELECT CampaignName, AdGroupId, AdGroupName, Clicks, Spend, Conversions, Impressions, Ctr " +
...
function ensureSeed_(cfg) {
  ...
  var keyword =
    (cfg.desired && cfg.desired.keyword) || '"digital certificates"';
  ...
  try {
    var businessDays = (
      cfg.business_days_csv || "MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY"
    ).split(",");
...
function initializeSafetyGuards_(cfg) {
  ...
  NEG_GUARD_ACTIVE =
    cfg.PROMOTE && !PREVIEW_MODE && RUN_MODE !== "IDEMPOTENCY_TEST";
...
function runIdempotencyTest_() {
  ...
  log_(
    "• Second run completed - " +
      secondRunResults.mutationCount +
      " mutations planned",
  );
  ...
  if (isIdempotent) {
    ...
    log_(
      "✗ IDEMPOTENCY TEST FAILED - Second run planned " +
        secondRunResults.mutationCount +
        " mutations",
    );
  ...
  try {
    postToBackend_("metrics", {
      ...
      search_terms: [],
      run_logs: [
        [new Date(), "IDEMPOTENCY_TEST_RESULT: " + JSON.stringify(testResult)],
      ],
    });
  } catch (e) {
