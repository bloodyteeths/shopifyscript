...
class CanaryRollbackManager {
  constructor(tenant, options = {}) {
    ...
    this.log("INFO", "Rollback manager initialized", {
      tenant,
  ...
  log(level, message, details = {}) {
    ...
    console.log(
      `[${level}] [ROLLBACK-${this.tenant}] ${message}`,
  ...
  async updateCurrentMetrics() {
    try {
      ...
      const errorResponse = await fetch(
        `http://localhost:3001/api/logs/${this.tenant}/errors`,
      );
      if (errorResponse.ok) {
  ...
  checkBudgetBreach() {
    ...
    return {
      ...
      details: {
        ...
        threshold,
        breachPercentage:
          dailyBudget > 0 ? (currentSpend / dailyBudget) * 100 : 0,
      },
  ...
  checkErrorFlood() {
    ...
    const triggered =
      recentErrorCount >= ROLLBACK_TRIGGERS.ERROR_FLOOD.threshold;
  ...
  checkPerformanceDrop() {
    ...
    const threshold =
      baselineCTR * ROLLBACK_TRIGGERS.PERFORMANCE_DROP.threshold;
  ...
  async executeImmediateRollback(trigger) {
    ...
    try {
      ...
      this.log("INFO", `Immediate rollback completed in ${rollbackTime}ms`, {
        rollbackId,
      });
    } catch (error) {
  ...
  async executeUrgentRollback(trigger) {
    ...
    try {
      ...
      this.log("INFO", `Urgent rollback completed in ${rollbackTime}ms`, {
        rollbackId,
      });
    } catch (error) {
      this.log("ERROR", "Urgent rollback failed", {
        error: error.message,
        rollbackId,
      });
      throw error;
  ...
  async pauseCampaign() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/campaigns/${this.tenant}/pause`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async clearAudienceMappings() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/audiences/${this.tenant}/clear`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ reason: "safety_rollback" }),
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to clear audience mappings: ${response.statusText}`,
        );
      }

      this.log("INFO", "Audience mappings cleared");
    } catch (error) {
      this.log("ERROR", "Failed to clear audience mappings", {
        error: error.message,
      });
      throw error;
    }
  ...
  async resetBudgetLimits() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/campaigns/${this.tenant}/reset-budget`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      this.log("ERROR", "Failed to reset budget limits", {
        error: error.message,
      });
      throw error;
    }
  ...
  async revertScheduleChanges() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/campaigns/${this.tenant}/reset-schedule`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!command || !tenant) {
    console.error(
      "Usage: node canary-rollback.js <command> <tenant> [options]",
    );
