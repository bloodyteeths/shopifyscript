...
class EnvironmentManager {
    ...
    for (const [key, schema] of Object.entries(ENV_SCHEMA)) {
      ...
      if (value !== undefined) {
        ...
        if (schema.allowed && !schema.allowed.includes(value)) {
          this.errors.push(`Invalid value for ${key}: must be one of [${schema.allowed.join(', ')}]`);
          continue;
        }

        // Custom validation
        if (schema.validate && !schema.validate(this._convertType(value, schema.type))) {
  ...
  _validateProductionSettings() {
    ...
    if (this.env.CORS_ORIGIN === '*') {
      this.warnings.push('CORS_ORIGIN set to wildcard (*) in production - consider restricting');
    }

    if (this.env.LOG_LEVEL === 'debug') {
      this.warnings.push('Debug logging enabled in production - consider changing to info or warn');
    }

    // Performance warnings
    if (parseInt(this.env.RATE_LIMIT_MAX_REQUESTS) > 1000) {
      this.warnings.push('Rate limit set very high - ensure this is intentional');
    }
  }
