...
class CanaryExecutor {
  constructor(tenant, config = {}) {
    ...
    this.log("INFO", "Canary executor initialized", {
      tenant,
      config: this.config,
    });
  }
  ...
  log(level, message, details = {}) {
    const entry = {
      ...
      elapsed: this.executionStartTime
    ...
    console.log(
      `[${level}] [EXECUTE-${this.tenant}] [${this.currentPhase || "INIT"}] ${message}`,
  ...
  async validateConfiguration(testConfig) {
    ...
    if (!validationReport.summary.passed) {
      throw new Error(
        `Configuration validation failed: ${validationReport.criticalErrors.length} critical errors`,
      );
    }

    if (this.config.strictValidation && validationReport.summary.warnings > 0) {
      throw new Error(
        `Strict validation failed: ${validationReport.summary.warnings} warnings detected`,
      );
    }
  }
  ...
  async validateAudience(audienceConfig) {
    ...
    const audienceReport =
      await this.audienceValidator.validateAudienceAttachment(audienceConfig);

    if (!audienceReport.passed) {
      throw new Error(
        `Audience validation failed: ${audienceReport.summary.errors} errors`,
      );
    }
  }
  ...
  async performSafetyChecks(testConfig) {
    ...
    if (testConfig.budgetCaps) {
      const maxBudget = Math.max(
        ...testConfig.budgetCaps.map((b) => parseFloat(b)),
      );
      if (maxBudget > 10) {
    ...
    if (this.config.windowDuration > 4 * 60 * 60 * 1000) {
      // 4 hours
      throw new Error(
        `Window duration too long: ${this.config.windowDuration}ms`,
      );
    }
  ...
  async collectBaseline(testConfig) {
    ...
    const response = await fetch(
      `http://localhost:3001/api/campaigns/${this.tenant}/baseline`,
    );
    if (response.ok) {
  ...
  async performPreflightTest(testConfig) {
    ...
    if (result.errors && result.errors.length > 0) {
      throw new Error(
        `Pre-flight test found errors: ${result.errors.join(", ")}`,
      );
    }
  }
  ...
  async waitForStartTime() {
    ...
    if (waitTime > 0) {
      this.log(
        "INFO",
        `Waiting ${Math.round(waitTime / 1000)}s for scheduled start time`,
      );
  ...
  async enablePromote(reason) {
    ...
    const response = await fetch(
      `http://localhost:3001/api/config/${this.tenant}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async verifyChangesApplied(testConfig) {
    ...
    const response = await fetch(
      `http://localhost:3001/api/google-ads/changes/${this.tenant}`,
    );
    if (response.ok) {
  ...
  async collectCurrentMetrics() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/campaigns/${this.tenant}/metrics`,
      );
      if (response.ok) {
  ...
  async verifyPromoteDisabled() {
    ...
    const response = await fetch(
      `http://localhost:3001/api/config/${this.tenant}/PROMOTE`,
    );
    if (response.ok) {
      const config = await response.json();
      if (config.value !== "FALSE") {
        this.log("WARNING", "PROMOTE flag not properly disabled", {
          value: config.value,
        });
      }
    }
  ...
  async analyzePerformance() {
    ...
    const analysis = {
      ...
      impressionsDelta:
        this.finalMetrics.impressions - (this.baselineMetrics.impressions || 0),
    };
  ...
  async saveExecutionData(report) {
    ...
    try {
      ...
      this.log("ERROR", "Failed to save execution data", {
        error: error.message,
      });
    }
  }
  ...
  generateRecommendations(success) {
    if (!success) {
      return {
        ...
        next: [
          ...
          "Re-run validation",
          "Consider smaller test window",
        ],
      };
    }

    const warningCount = this.alertLog.filter(
      (a) => a.level === "WARNING",
    ).length;
    ...
    if (warningCount > 5) {
      return {
        ...
        next: [
          "Address warnings before scaling",
