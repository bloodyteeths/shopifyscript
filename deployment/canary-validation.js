...
const SAFETY_LIMITS = {
  MAX_DAILY_BUDGET: 10.00,      // Absolute maximum for canary
  MAX_CPC_CEILING: 0.50,        // Absolute maximum CPC
...
class CanaryValidator {
  ...
  log(level, message, details = {}) {
    ...
    console.log(`[${level}] ${message}`, Object.keys(details).length ? details : '');
  }
  ...
  async validateEnvironment() {
    ...
    for (const envVar of requiredEnvVars) {
      if (!process.env[envVar]) {
        this.log(VALIDATION_LEVELS.CRITICAL, `Missing required environment variable: ${envVar}`);
      }
    }
    
    // Validate HMAC secret length
    if (process.env.HMAC_SECRET && process.env.HMAC_SECRET.length < 32) {
      this.log(VALIDATION_LEVELS.CRITICAL, 'HMAC_SECRET must be at least 32 characters', {
    ...
    if (process.env.GEMINI_API_KEY && !process.env.GEMINI_API_KEY.startsWith('AIza')) {
      this.log(VALIDATION_LEVELS.CRITICAL, 'Invalid GEMINI_API_KEY format - must start with AIza');
    }
  ...
  validateSafetyConfiguration(config) {
    ...
    if (config.PROMOTE === true || config.PROMOTE === 'TRUE') {
      this.log(VALIDATION_LEVELS.CRITICAL, 'PROMOTE must be FALSE during setup phase', {
  ...
  validateBudgetSafety(budgetCaps) {
    ...
    for (const cap of budgetCaps) {
      ...
      if (isNaN(dailyBudget)) {
        this.log(VALIDATION_LEVELS.CRITICAL, 'Invalid budget cap format', { cap });
        continue;
      }
      
      if (dailyBudget > SAFETY_LIMITS.MAX_DAILY_BUDGET) {
        this.log(VALIDATION_LEVELS.CRITICAL, 'Budget cap exceeds safety limit', {
          cap: dailyBudget,
      ...
      if (dailyBudget < 1.00) {
        this.log(VALIDATION_LEVELS.WARNING, 'Budget cap very low - may limit meaningful testing', {
          cap: dailyBudget
        });
      }
    }
  ...
  validateCPCSafety(cpcCeilings) {
    ...
    for (const ceiling of cpcCeilings) {
      ...
      if (isNaN(maxCPC)) {
        this.log(VALIDATION_LEVELS.CRITICAL, 'Invalid CPC ceiling format', { ceiling });
        continue;
      }
      
      if (maxCPC > SAFETY_LIMITS.MAX_CPC_CEILING) {
        this.log(VALIDATION_LEVELS.CRITICAL, 'CPC ceiling exceeds safety limit', {
          ceiling: maxCPC,
      ...
      if (maxCPC < 0.05) {
        this.log(VALIDATION_LEVELS.WARNING, 'CPC ceiling very low - may prevent ad serving', {
          ceiling: maxCPC
        });
      }
    }
  ...
  validateScheduleSafety(schedules) {
    ...
    if (currentHour < 9 || currentHour >= 17) {
      this.log(VALIDATION_LEVELS.WARNING, 'Canary scheduled outside business hours', {
        currentHour,
    ...
    if (currentDay === 0 || currentDay === 5 || currentDay === 6) {
      this.log(VALIDATION_LEVELS.WARNING, 'Canary scheduled on weekend or Friday', {
    ...
    for (const schedule of schedules) {
      ...
      if (parts.length >= 3) {
        ...
        if (isNaN(duration)) {
          this.log(VALIDATION_LEVELS.CRITICAL, 'Invalid schedule duration format', { schedule });
          continue;
        }
        
        if (duration > SAFETY_LIMITS.MAX_WINDOW_MINUTES) {
          this.log(VALIDATION_LEVELS.CRITICAL, 'Schedule duration exceeds safety limit', {
            duration,
        ...
        if (duration < 30) {
          this.log(VALIDATION_LEVELS.WARNING, 'Schedule duration very short for meaningful testing', {
            duration,
  ...
  validateCampaignExclusions(exclusions, campaignName) {
    ...
    if (!exclusions || exclusions.length === 0) {
      this.log(VALIDATION_LEVELS.WARNING, 'No campaign exclusions configured - all campaigns may be affected');
      return;
    }
    
    // Check if canary campaign is in exclusions (it shouldn't be)
    const excludedCampaigns = exclusions.join(',').split(',').map(c => c.trim());
    
    if (excludedCampaigns.includes(campaignName)) {
      this.log(VALIDATION_LEVELS.CRITICAL, 'Canary campaign found in exclusions list', {
        campaignName,
    ...
    if (excludedCampaigns.length === 0) {
      this.log(VALIDATION_LEVELS.WARNING, 'No campaigns excluded - ALL campaigns will be affected');
    }
  }
  ...
  validateAudienceSafety(audienceMap) {
    ...
    if (!audienceMap || audienceMap.length === 0) {
      this.log(VALIDATION_LEVELS.INFO, 'No audience mapping configured - test will run without audience targeting');
      return;
    }
    ...
    for (const mapping of audienceMap) {
      ...
      if (mapping.mode !== 'OBSERVE') {
        this.log(VALIDATION_LEVELS.CRITICAL, 'Audience mode must be OBSERVE for canary testing', {
      ...
      if (Math.abs(bidMod) > 0.25) {
        this.log(VALIDATION_LEVELS.WARNING, 'Large bid modifier may skew canary results', {
          bidModifier: bidMod,
  ...
  validatePromoteWindow(windowConfig) {
    ...
    if (bufferMinutes < SAFETY_LIMITS.REQUIRED_BUFFER_MINUTES) {
      this.log(VALIDATION_LEVELS.WARNING, 'Insufficient buffer time before window start', {
    ...
    // Check duration
    if (duration > SAFETY_LIMITS.MAX_WINDOW_MINUTES) {
      this.log(VALIDATION_LEVELS.CRITICAL, 'Window duration exceeds safety limit', {
        duration,
        maxAllowed: SAFETY_LIMITS.MAX_WINDOW_MINUTES
      });
    }
    ...
    if (endTime.getHours() >= 18) {
      this.log(VALIDATION_LEVELS.WARNING, 'Window may extend beyond business hours', {
  ...
  async runFullValidation(configData) {
    this.log(VALIDATION_LEVELS.INFO, `Starting comprehensive canary validation for tenant: ${this.tenant}`);
    ...
    try {
      ...
      this.validateCampaignExclusions(configData.exclusions || [], configData.campaignName);
      ...
      this.log(VALIDATION_LEVELS.CRITICAL, 'Validation process failed with error', {
        error: error.message,
        stack: error.stack
      });
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  validator.runFullValidation(configData)
    .then(report => {
      console.log('\n=== VALIDATION REPORT ===');
