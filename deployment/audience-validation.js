...
class AudienceValidator {
  constructor(tenant, options = {}) {
    ...
    this.log('INFO', 'Audience validator initialized', { tenant, options: this.options });
  }
  ...
  log(level, message, details = {}) {
    ...
    console.log(`[${level}] [AUDIENCE-${this.tenant}] ${message}`, 
  ...
  async validateAudienceAttachment(audienceConfig) {
    ...
    try {
      ...
      const listValid = await this.validateGoogleAdsList(audienceConfig.userListId);
      if (!listValid && this.options.strictMode) return this.generateReport(false);
      
      // Phase 3: Customer Data Validation
      if (audienceConfig.customerData) {
        const dataValid = await this.validateCustomerData(audienceConfig.customerData);
      ...
      const sizeValid = await this.validateAudienceSize(audienceConfig.userListId);
      if (!sizeValid && this.options.strictMode) return this.generateReport(false);
  ...
  validateAudienceConfiguration(config) {
    ...
    if (config.mode !== AUDIENCE_SAFETY_RULES.REQUIRED_MODE) {
      this.log('ERROR', `Invalid targeting mode - must be ${AUDIENCE_SAFETY_RULES.REQUIRED_MODE} for canary`, {
    ...
    if (Array.isArray(config.userListId) && config.userListId.length > AUDIENCE_SAFETY_RULES.MAX_LISTS_PER_CAMPAIGN) {
      this.log('WARNING', 'Multiple audience lists may complicate canary analysis', {
  ...
  async validateGoogleAdsList(userListId) {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/google-ads/audience/${userListId}`);
      ...
      if (!this.googleAdsListInfo.exists) {
        this.log('ERROR', 'Audience list does not exist in Google Ads', { userListId });
        return false;
      }
      ...
      if (!this.isCompatibleListType(this.googleAdsListInfo.type)) {
        this.log('ERROR', 'Incompatible audience list type for canary testing', {
      ...
      if (!this.googleAdsListInfo.canTarget) {
        this.log('ERROR', 'Insufficient permissions to target this audience list', { userListId });
        return false;
      }
  ...
  validateCustomerRecord(record) {
    ...
    if (this.options.enforceGDPR) {
      ...
      if (record.consentTimestamp) {
        const consentAge = (Date.now() - new Date(record.consentTimestamp).getTime()) / (1000 * 60 * 60 * 24);
  ...
  validateGDPRCompliance(config) {
    ...
    if (!config.dataRetentionDays || config.dataRetentionDays > 365) {
      this.log('WARNING', 'Data retention period may exceed GDPR recommendations', {
  ...
  async validateAudienceSize(userListId) {
    ...
    if (listSize < AUDIENCE_SAFETY_RULES.MIN_LIST_SIZE) {
      this.log('WARNING', 'Audience size below recommended minimum for testing', {
        listSize,
  ...
  validateAttachmentSafety(config) {
    ...
    if (config.mode !== 'OBSERVE') {
      this.log('ERROR', 'Canary tests must use OBSERVE mode for audience attachment');
      valid = false;
    }
    ...
    if (bidModifier < -0.25) {
      this.log('ERROR', 'Negative bid modifier too large for safe canary testing', {
        bidModifier,
        minSafe: -0.25
      });
      valid = false;
    ...
    if (!this.isCanaryCampaign(config.campaignName)) {
      this.log('ERROR', 'Audience can only be attached to designated canary campaign', {
    ...
    if (config.exclusions && config.exclusions.length > 0) {
      this.log('WARNING', 'Audience exclusions may complicate canary analysis', {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  validator.validateAudienceAttachment(config)
    .then(report => {
